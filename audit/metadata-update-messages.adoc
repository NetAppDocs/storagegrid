...
---
= Audit messages for S3 object metadata updates in StorageGRID
:icons: font
:imagesdir: ../media/
...
The S3 client makes a request (SUPD) to update the specified metadata (`x-amz-meta-*`) for the S3 object (S3KY). 

In this example, request headers are included in the field HTRH because it has been configured as an audit protocol header (*Configuration* > *Monitoring* > *Audit and syslog server*). See link:../monitor/configure-log-management.html[Configure log management and external syslog server].
