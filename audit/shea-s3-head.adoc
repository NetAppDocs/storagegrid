...
summary: 'When an S3 client issues a HEAD operation, a request is made to check for the existence of an object or bucket and retrieve the metadata about an object. This message is issued by the server if the operation is successful.'
---
= SHEA audit message for S3 HEAD operations in StorageGRID
:icons: font
:imagesdir: ../media/
...
| ASRS
| Assumed Role Session URN
| The unique identifier of the assumed role session.

Present only when the S3 request is using temporary security credentials from an AssumeRole request.

| ASRU
| Assumed Role URN
| The unique identifier of the assumed role.
