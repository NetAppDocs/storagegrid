...
Audit messages are saved on the nodes that generated them (local node) by default. The audit information generated on every local node is stored in /var/local/log/localaudit.log.

If you select "AdminNodes/local Nodes", audit message are stored on the Admin Node in /var/local/audit/export/audit.log.

Audit messages are processed by those Storage Nodes and Admin Nodes that have an Administrative Domain Controller (ADC) service
