...
The following workflows for audit messages and audit message retention is only applicable if StorageGRID is configured for "Admin Nodes/local nodes" or "Admin Node and external syslog server". If StorageGRID is configured for "Local nodes only" (default) or "External syslog server", the audit messages are saved locally on the node in the `/var/local/log/localaudit.log` file and can't be processed by Admin Nodes or Storage Nodes.
