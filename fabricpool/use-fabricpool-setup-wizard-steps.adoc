...
---
= Access and complete the StorageGRID FabricPool setup wizard
:icons: font
:imagesdir: ../media/
...
. Define root access for the tenant account, based on whether your StorageGRID system uses link:../admin/using-identity-federation.html[identity federation], link:../admin/how-sso-works.html[single sign-on (SSO)], or both. 
+
[cols="1a,3a" options="header"]
|===
| Option 
...
. Optionally, select the copy icon (image:icon_tenant_copy_url.png[copy icon]) to copy both the access key ID and secret access key to the clipboard.
+
