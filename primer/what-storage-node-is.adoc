...
When selecting which Storage Nodes to use in your topology, keep in mind that the grid and each site in the grid must contain the following:

* Three <<adc-service,ADC>> Storage Nodes per site (can be any combination of combined and metadata-only Storage Nodes)
* Single site grid: At least two object storage nodes (can be any combination of combined and data-only Storage Nodes)
* Multi-site grid: At least one object storage node per site (can be either combined or data-only Storage Node)
