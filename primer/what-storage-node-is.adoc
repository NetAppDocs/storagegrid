...
* When using software-based nodes, you must ensure that metadata-only node resources match the resources of the existing Storage Nodes. For example: 
...
* Per site (in a single- or multi-site grid): Three <<adc-service,ADC>> Storage Nodes (can be any combination of combined and metadata-only Storage Nodes)
