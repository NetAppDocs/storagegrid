...
You can convert a combined Storage Node to a data-only Storage Node. If the Storage Node contains the <<adc-service,ADC service>>, the service is automatically moved to another Storage Node on the same site during the conversion. Refer to link:../maintain/convert-to-data-only-node.html[Convert a Storage Node to a data-only node].
