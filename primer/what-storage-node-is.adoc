...
* After you decommission and then add a new Storage Node

NOTE: You can't change the type after the Storage Node installation is complete.

Data and metadata Storage Node::
By default, all new Storage Nodes installed will store both object data and metadata.
...
* For a single-site grid, configure at least two Storage Nodes for object data and metadata. 
* For a multi-site grid, configure at least one Storage Node _per site_ for object data and metadata.
...
Data-only Storage Node::
Using a Storage Node exclusively for metadata can make sense if.............

When installing data-only nodes, the grid must also contain a minimum number of nodes for metadata storage:
