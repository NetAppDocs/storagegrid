...
---
= What is a StorageGRID Storage Node?
:icons: font
:imagesdir: ../media/
...
By default, all new Storage Nodes store both object data and metadata. This type of Storage Node is called a _combined_ Storage Node.
...
* When using StorageGRID appliances, you can only configure metadata-only nodes on SGF6112 appliances with twelve 1.9 TB or twelve 3.8 TB drives.
* When using software-based nodes, you must ensure that metadata-only node resources match the existing Storage Nodes resources. For example: 
...
* When adding a new StorageGRID site, the new site total metadata capacity should, at a minimum, match existing sites. The resources at a new site should match the Storage Nodes at existing sites.
...
You can convert a combined Storage Node to a data-only Storage Node. If the Storage Node contains the <<adc-service,ADC service>>, and the prerequisites for conversion are met, the service is automatically moved to another Storage Node on the same site during the conversion. For prerequisites and detailed steps, refer to link:../maintain/convert-to-data-only-node.html[Convert a Storage Node to a data-only node].

== Required Storage Nodes per grid and per site

When selecting which Storage Nodes to use in your topology, keep in mind that the grid or each site in the grid must contain the following:

* Per site (in a single- or multi-site grid): Three <<adc-service,ADC>> Storage Nodes (can be any combination of combined, metadata-only, and data-only Storage Nodes)
* Single-site grid: At least two object Storage Nodes (can be any combination of combined and data-only)
* Multi-site grid: At least one object Storage Node per site (can be either combined or data-only)
...
Data-only Storage Nodes don't host this service.
