...
| gdu-server
| The backend task runner for grid maintenance and provisioning actions. Accepts requests from `mgmt-api` and other utility scripts, validates them, and dispatches each action to the appropriate GDU task handler, such as provisioning, expansion, decommission, and upgrades. Logs results and returns structured responses.
| Admin Nodes
