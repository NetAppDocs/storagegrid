...
---
= StorageGRID grid nodes and services
:icons: font
:imagesdir: ../media/
...
Admin Nodes:: Provide management services such as system configuration, monitoring, and logging. When you sign in to the Grid Manager, you are connecting to an Admin Node. Each grid must have one primary Admin Node and might have additional non-primary Admin Nodes for redundancy. You can connect to any Admin Node, and each Admin Node displays a similar view of the StorageGRID system. However, you must perform most maintenance procedures using the primary Admin Node.
+
...
+
During initial installation of a new Storage Node, you can specify that it only be used to link:../primer/what-storage-node-is.html#types-of-storage-nodes[store either metadata or data].
+
...
| gdu-server
| The backend task runner for grid maintenance and provisioning actions. Accepts requests from `mgmt-api` and other utility scripts, validates them, and dispatches each action to the appropriate GDU task handler, such as provisioning, expansion, decommission, and upgrades.
| Admin Nodes
...
| High Availability
| Manages high availability virtual IPs on nodes configured on the High Availability Groups page. This service is also known as the _keepalived_ service.
...
| mgmt-api
| The Grid Manager backend API. Exposes authenticated REST endpoints for administering the grid (configuration, monitoring, and maintenance workflows), enforces validations and permissions, and orchestrates long-running operations. The primary control-plane API used by the Grid Manager interface and automation clients.
| Admin Nodes
