...
== [[block-remote-ipmi]]Control remote IPMI access to BMC

You can enable or disable remote IPMI access for all appliances containing a BMC. The remote IPMI interface allows low-level hardware access to your StorageGRID appliances by anyone with a BMC account and password. If you don't need remote IPMI access to the BMC, disable this option.

* To control remote IPMI access to the BMC in Grid Manager, go to *Configuration* > *Security* > *Security settings* > *Appliances* > *Remote IPMI access*:
...
For additional information about BMC hardening, see the https://media.defense.gov/2023/Jun/14/2003241405/-1/-1/0/CSI_HARDEN_BMCS.PDF[Harden Baseboard Management Controllers^] cybersecurity information sheet from the https://www.nsa.gov/[National Security Agency (NSA)^] and https://www.cisa.gov/[Cybersecurity and Infrastructure Security Agency (CISA)^].

== Change appliance BMC password

You can change the BMC password for an appliance to limit access and improve system security.

. In Grid Manager go to *Nodes* > *_grid node_* > *BMC settings*.
. In the Change password section, enter a new BMC password between 8 and 16 characters in length in *New password* and then enter it again in *Confirm new password*.
. Select *Save*.

You can also change the BMC password using the `PUT /bmc-configuration/root-password` API.

== [[control-remote-access-through-usb-ports]]Control external USB port access on appliances

You can enable the USB ports on the appliances to allow technical support access for diagnostics and troubleshooting. If you don't need USB port access, disable the ports for security. USB ports are enabled by default after the initial install or an upgrade.

* In the Grid Manager, go to *Configuration* > *Security* > *Security settings* > *Appliances* > *USB port access* to enable or disable the USB ports:
** Select the *Enable USB ports* checkbox and save to enable the ports.
+
If the USB checkbox was previously unchecked and saved, select the *Enable USB ports* checkbox and save to enable USB ports again.
** Clear the *Enable USB ports* checkbox and save to disable the ports.
...
For a list of all internal and external ports used by StorageGRID, see link:../network/internal-grid-node-communications.html#storagegrid-internal-ports[StorageGRID internal ports] and link:../network/external-communications.html#ports-used-for-external-communications[Ports used for external communications].
...
You must limit physical access to StorageGRID hardware appliance nodes as well as VMware virtual machine hosts and Linux hosts running StorageGRID to authorized administrators only. Some examples of physical access controls include locks, guards, physical barriers, and video surveillance.

Hardware appliance nodes are designed to be installed and operated only by authorized administrators. Don't allow un-authorized administrators to access hardware appliance nodes.
...
* If the appliance has a baseboard management controller (BMC), be aware that the BMC management port allows low-level hardware access. Connect the BMC management port only to a secure, trusted, internal management network.
+
You can establish a VLAN to isolate BMC network connections and restrict BMC internet access to trusted networks. For additional information about enforcing VLAN separation, see the https://media.defense.gov/2023/Jun/14/2003241405/-1/-1/0/CSI_HARDEN_BMCS.PDF[Harden Baseboard Management Controllers^] cybersecurity information sheet from the https://www.nsa.gov/[National Security Agency (NSA)^] and https://www.cisa.gov/[Cybersecurity and Infrastructure Security Agency (CISA)^].
+
If a secure, trusted, internal management network isn't available, leave the BMC management port unconnected or blocked. Technical support might request temporary access during a support case.
...
+
You can also <<block-remote-ipmi,disable remote IPMI access>>.
+
...
* For appliance models containing SED or FIPS NVMe SSDs that you manage using the StorageGRID Appliance Installer and Grid Manager, https://docs.netapp.com/us-en/storagegrid-appliances/installconfig/optional-enabling-node-encryption.html#enable-drive-encryption[enable and configure StorageGRID drive encryption^].

* For appliances without SED, FDE, or FIPS drives, use a Key Management Server (KMS) to  https://docs.netapp.com/us-en/storagegrid-appliances/installconfig/optional-enabling-node-encryption.html#enable-node-encryption[enable and configure StorageGRID software node encryption^].

.Related information
https://docs.netapp.com/us-en/e-series-santricity/sm-settings/overview-drive-security.html[Learn about drive security in SANtricity System Manager^]
