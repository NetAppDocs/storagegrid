...
+
You can establish a VLAN to isolate BMC network connections to limit the endpoints that can communicate with the BMC and restrict BMC internet access. For additional information about enforcing VLAN separation, see the https://media.defense.gov/2023/Jun/14/2003241405/-1/-1/0/CSI_HARDEN_BMCS.PDF[Harden Baseboard Management Controllers^] cybersecurity information sheet from the https://www.nsa.gov/[National Security Agency (NSA)] and https://www.cisa.gov/[Cybersecurity and Infrastructure Security Agency (CISA)^].
+
If a secure, trusted, internal management network is not available, leave the BMC management port unconnected or blocked, unless a BMC connection is requested by technical support.
