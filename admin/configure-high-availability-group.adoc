...
---
= Configure high availability groups in StorageGRID
:icons: font
:imagesdir: ../media/
...
NOTE: A StorageGRID system can have a maximum of 255 HA groups.
...
** *VMware (after installing the node)*: link:../maintain/vmware-adding-trunk-or-access-interfaces-to-node.html[Add trunk or access interfaces to a node]
...
+
image:vlan_parent_interface_tooltip.png[VLAN Interface tool tip]
...
+
image:ha_group_determine_failover.png[HA Groups determine priority order]
+
...
+
image:ha_group_select_virtual_ips.png[HA Groups enter VIPs]
