...
** Create new S3 access keys, which can be optionally synced to other connected grids. See link:../tenant/grid-federation-clone-keys-with-api.html[Sync S3 access keys using the API].
...
* If a tenant has the *Use grid federation connection* permission, you can edit tenant details from either grid in the connection. Changes to tenant groups, local users, and S3 access keys owned by local users are automatically synced to all connected grids. However, changes to tenant-level settings (such as name, description, or quota) are not automatically synced; you must make those edits on each grid.
