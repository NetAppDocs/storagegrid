...
summary: "You can allow S3 tenant accounts to use one or more grid federation connections among multiple StorageGRID systems. When tenants are allowed to use a connection, special steps are required to edit tenant details or to permanently remove tenant permission to use the connection."
---
= Manage the permitted tenants for StorageGRID grid federation
:icons: font
:imagesdir: ../media/

[.lead]
You can allow S3 tenant accounts to use one or more grid federation connections among multiple StorageGRID systems. When tenants are allowed to use a connection, special steps are required to edit tenant details or to permanently remove a tenant's permission to use the connection.
...
* You have reviewed the workflows for link:grid-federation-what-is-account-sync.html[account sync] and link:grid-federation-what-is-cross-grid-replication.html[cross-grid replication].
* As required, you have already configured single sign-on (SSO) or identity federation for both grids in the connection. See link:grid-federation-what-is-account-sync.html[Learn about account sync for grid federation].

== Create a permitted tenant
If you want to allow a new or existing tenant account to use a grid federation connection for account sync and cross-grid replication, follow the general instructions to link:creating-tenant-account.html[create a new S3 tenant] or link:editing-tenant-account.html[edit a tenant account] and note the following:
...
** From any connected grid, create groups and local users, which are automatically synced to all other connected grids. See link:../tenant/grid-federation-account-sync.html[Sync tenant groups and users].

** Create new S3 access keys for local users, which are automatically synced to other connected grids when access key sync is enabled for the tenant.
+
NOTE: S3 access keys for federated users are cloned one time from source to target when they're created. If these S3 access keys are deleted, these changes aren't synced to other grids. You must manually delete the S3 access keys on the other grids.
...
. Optionally select the *Grid federation* tab to link:../monitor/grid-federation-monitor-connections.html[monitor the grid federation connections].
...
* If a tenant has the *Use grid federation connection* permission:

* You can edit tenant details from any grid in the connection. Changes to tenant-level settings (such as name, description, or quota) as well as groups, local users, and S3 access keys owned by local users are automatically synced to all connected grids.

* You can change *Allowed grid federation connections* _only on the source grid_.

* You can add a new grid federation connection.
+
NOTE: You can't unselect any existing connections. To remove a connection, go to the details page for the tenant, select the *Grid federation* tab, and remove the permission.

* If you add a new connection while editing a tenant, this tenant will be replicated to the remote grid specified by that new connection.

== Delete a permitted tenant

CAUTION: If you remove either the original tenant from the source grid or the synced tenant from the destination grid, the tenant is removed from all the grids, so the account can no longer be used.
...
* Before you can remove the tenant on the source grid, you must remove all buckets for the account on the source grid.

* Before you can remove the synced tenant on the destination grid, you must remove all buckets for the account on the destination grid.

* If you remove either the original or the synced tenant, the account can no longer be used for cross-grid replication. 

* If you are removing the original tenant on the source or any destination grid, tenant groups, users, and keys that were synced to the destination grids are also removed.
...
image:grid-federation-remove-permission.png[steps to remove grid federation connection]
...
NOTE: Re-enabling the *Use grid federation connection* permission makes the local grid the source grid and triggers syncing to the remote grid specified by the selected grid federation connection. If the tenant account already exists on the remote grid, syncing will result in a conflict error.
...
+
image:grid-federation-remove-permission-error.png[error message shown if tenant has cgr enabled for a bucket]
+
