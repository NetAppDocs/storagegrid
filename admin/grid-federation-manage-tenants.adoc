...
** Create new S3 access keys for local users, which are automatically synced to other connected grids.
+
NOTE: S3 access keys for federated users are synced one time when they're created. If these S3 access keys are created or deleted, these changes aren't synced to other grids. You must manually create or delete the S3 access keys on the other grids.
