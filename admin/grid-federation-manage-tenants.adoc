...
* You have reviewed the workflows for link:grid-federation-what-is-account-clone.html[account sync] and link:grid-federation-what-is-cross-grid-replication.html[cross-grid replication].
* As required, you have already configured single sign-on (SSO) or identity federation for both grids in the connection. See link:grid-federation-what-is-account-clone.html[Learn about account sync for grid federation].
...
** From any connected grid, create groups and local users, which are automatically synced to all other connected grids. See link:../tenant/grid-federation-account-clone.html[Sync tenant groups and users].

** Create new S3 access keys for local users, which are automatically synced to other connected grids. Refer to link:../tenant/grid-federation-clone-keys-with-api.html[Sync S3 access keys using the API].
+
NOTE: S3 access keys for federated users are cloned one time when they're created. If these S3 access keys are edited or deleted, these changes aren't synced to other grids. You must manually edit or delete the S3 access keys on the other grids.
...
+ 
If this is the source grid for the tenant (that is, if the tenant was created on this grid), a banner appears to remind you that the tenant was synced to another grid. Edits to tenant groups, users, and S3 access keys are automatically synced to all connected grids.
+
...
* If a tenant has the *Use grid federation connection* permission, you can edit tenant details from any grid in the connection. Changes to tenant groups, local users, and S3 access keys owned by local users are automatically synced to all connected grids. However, changes to tenant-level settings (such as name, description, or quota) aren't automatically synced; you must make those edits on each grid.

* You can't clear the *Use grid federation connection* permission when you're editing a tenant.

* You can't select a different grid federation connection when you're editing a tenant.
...
* If you are removing the original tenant on the source grid, any tenant groups, users, or keys that were synced to the destination grid will be unaffected. You can either delete the synced tenant or allow it to manage its own groups, users, access keys, and buckets.

* If you are removing the synced tenant on the destination grid, sync errors will occur if new groups or users are added to the original tenant.
+
...
NOTE: *DOES THIS NOTE NEED TO CHANGE?* Re-enabling the *Use grid federation connection* permission makes the local grid the source grid and triggers syncing to the remote grid specified by the selected grid federation connection. If the tenant account already exists on the remote grid, syncing will result in a conflict error.
