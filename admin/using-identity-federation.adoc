...
---
= Use identity federation in StorageGRID
:icons: font
:imagesdir: ../media/
...
You can configure identity federation in the Grid Manager if you want admin groups and users to be managed in another system such as Active Directory, Microsoft Entra ID, OpenLDAP, or Oracle Directory Server.

.Before you begin
* You're signed in to the Grid Manager using a link:../admin/web-browser-requirements.html[supported web browser].
* You've link:admin-group-permissions.html[specific access permissions].
* You're using Active Directory, Microsoft Entra ID, OpenLDAP, or Oracle Directory Server as the identity provider.
+
NOTE: If you want to use an LDAP v3 service that isn't listed, contact technical support.
* If you plan to use OpenLDAP, you must configure the OpenLDAP server. See <<Guidelines for configuring an OpenLDAP server>>.
* If you plan to enable single sign-on (SSO), you've reviewed the link:requirements-for-sso.html[requirements and considerations for single sign-on].
...
.About this task
You can configure an identity source for the Grid Manager if you want to import groups from another system such as Active Directory, Microsoft Entra ID, OpenLDAP, or Oracle Directory Server. You can import the following types of groups:
