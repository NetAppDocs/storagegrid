...
---
= Use the StorageGRID API if single sign-on is enabled (PingFederate)
:icons: font
:imagesdir: ../media/

[.lead]
If you have link:../admin/how-sso-works.html[configured and enabled single sign-on (SSO)] and you use PingFederate as the SSO provider, you must issue a series of API requests to obtain an authentication token that is valid for the Grid Management API or the Tenant Management API.
...
+
image:sso_auth_python_script_ping.png[SSO Authentication Script for Ping Federate]
+
