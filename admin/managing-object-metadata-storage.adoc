...
---
= Manage object metadata storage in StorageGRID
:icons: font
:imagesdir: ../media/
...
image:metadata_space_across_sites.png[Metadata Space Across Sites]
...
The default setting for a new StorageGRID 12.1 or higher installation in which each Storage Node has 128 GB or more of RAM is 20,000 GB (20 TB).
...
+
image:storage_used_object_metadata_actual_reserved.png[Storage Used - Object Metadata - Actual Reserved]

In the screenshot, the Actual reserved value is 20 TB. This screenshot is for a large Storage Node in a new StorageGRID 12.1 installation. Volume 0 is at least 20 TB, so it uses the default system-wide Metadata reserved space setting.

=== Example for Actual reserved metadata space

Suppose you install a new StorageGRID system using version 12.1 or later. For this example, assume that each Storage Node has more than 128 GB of RAM and that volume 0 of Storage Node 1 (SN1) is 6 TB. Based on these values:

* The system-wide *Metadata reserved space* is set to 20 TB. (This is the default value for a new StorageGRID 12.1 or higher installation if each Storage Node has more than 128 GB RAM.)
...
image:metadata_allowed_space_volume_0.png[Metadata allowed space volume 0]
...
|&lt;= 4 TB
|75% of actual reserved space for metadata, up to a maximum of 1.32 TB
|75% of actual reserved space for metadata, up to a maximum of 1.98 TB

|&gt; 4 TB
|(Actual reserved space for metadata − 1 TB) × 75%, up to a maximum of 1.32 TB
|(Actual reserved space for metadata − 1 TB) × 75%, up to a maximum of 7.92 TB
...
+
image:storage_used_object_metadata_allowed.png[Storage Used - Object Metadata - Allowed]

In the screenshot, the *Allowed* value is 7.92 TB, which is the maximum value for a Storage Node whose actual reserved space for metadata is more than 4 TB.
...
* The system-wide *Metadata reserved space* is set to 20 TB. (This is the default value for a new StorageGRID 12.1 or higher installation if each Storage Node has more than 128 GB RAM.)
* The *Actual reserved* space for metadata for SN1 is 6 TB. (The entire volume is reserved because volume 0 is smaller than the *Metadata reserved space* setting.)
* The *Allowed* space for metadata on SN1 is 3.75 TB, based on the calculation shown in the <<table-allowed-space-for-metadata,table for allowed space for metadata>>: (Actual reserved space for metadata − 1 TB) × 75%, up to a maximum of 7.92 TB.
...
image:metadata_space_three_storage_nodes.png[Metadata Space Three Storage Nodes]
