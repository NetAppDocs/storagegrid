...
Before changing SSH access passwords, download the current Recovery Package. You can use the passwords in this file if the password change process fails for any node.
...
. When the confirmation dialog appears, select *Yes* if you are ready to start changing the SSH access passwords.
+
...
.. Open the `.zip` file.
.. Confirm that you can access the contents, including the `Passwords.txt` file, which contains the new SSH access passwords.
...
.. Select *Retry*.
+
NOTE: Retrying only changes the SSH access passwords on the nodes that failed during previous password change attempts. 
