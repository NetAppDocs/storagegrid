...
| S3 Object Lock
| Allows the tenant to specify retention and legal hold settings for the objects ingested into individual buckets.

| Set maximum retention period
| Defines how long new objects added to this bucket should be retained, starting from the time they are ingested.

| Allow compliance mode on the tenant
| Users can't overwrite or delete protected object versions during the retention period.
|===
