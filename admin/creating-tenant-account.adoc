...
---
= Create a StorageGRID tenant account
:icons: font
:imagesdir: ../media/
...
The steps for creating a tenant account vary based on whether link:using-identity-federation.html[identity federation] and link:how-sso-works.html[single sign-on] are configured and whether the Grid Manager account you use to create the tenant account belongs to an admin group with the Root access permission.
...
* If you want to allow an S3 tenant to sync account data and replicate bucket objects to another grid using a grid federation connection:
...
| The tenant can use grid federation connections, which:

* Causes this tenant account and all tenant groups, users, and S3 access keys to be synced between this grid and the other grids in the selected connections.
+
TIP: If needed for security or compliance reasons, you can deselect *Enable access key sync*.
...
. If you selected the *Use grid federation connection* permission for the tenant, confirm that an identical tenant was replicated to the other grid in the connection. The tenants on both grids will have the same 20-digit account ID, name, description, quota, and permissions.
+
NOTE: If you see an error message that indicates the tenant account was created on this grid but wasn't synced to the other grids, refer to the instructions in link:grid-federation-troubleshoot.html[Troubleshoot grid federation errors].
...
Follow the instructions in link:../tenant/index.html[Use a tenant account] to manage tenant groups and users, S3 access keys, buckets, platform services, and account sync and cross-grid replication.