...
---
= Configure StorageGRID load balancer endpoints
:icons: font
:imagesdir: ../media/
...
Each S3 client load balancer endpoint specifies a port, a client type (S3), and a network protocol (HTTP or HTTPS). Management interface load balancer endpoints specify a port, interface type, and untrusted Client Network.
...
. To create an endpoint for an S3 client, select the *S3 client* tab.
...
If you're configuring load balancer endpoints for a link:../admin/configure-global-namespace.html[global namespace], use the same port for each endpoint.
...
| Enable caching
| Enable or disable link:../admin/managing-load-balancing.html#considerations-for-load-balancer-caching[caching on the Gateway Nodes] for this load balancer endpoint.

If issues occur with caching, refer to link:../troubleshoot/troubleshooting-load-balancer-caching.html[Troubleshoot load balancer caching].
|===
...
+
image:load_balancer_endpoint_details.png[Load balancer endpoint details]
...
.. Select the edit icon image:icon_edit_tm.png[Edit icon].
.. Enter the new name.
.. Select *Save*.
