...
summary: "You can use grid federation to sync tenants and replicate their objects among multiple StorageGRID systems for disaster recovery."
---
= Learn about StorageGRID grid federation
:icons: font
:imagesdir: ../media/

[.lead]
You can use grid federation to sync tenants and replicate their objects among multiple StorageGRID systems for disaster recovery.
...
image:grid-federation-workflow.png[Grid federation connection workflow]
...
+
For details about version requirements, refer to the https://library.netapp.com/ecm/ecm_download_file/ECMLP3373065[Release notes^]. You must sign in with your NetApp account or create an account to access the Release notes.
...
* At least one grid federation connection must exist before you can use link:grid-federation-what-is-account-sync.html[account sync] or link:grid-federation-what-is-cross-grid-replication.html[cross-grid replication].
...
image:grid-federation-certificates.png[Grid federation certificates]
