...
[.lead]
You use PingFederate to create a service provider (SP) connection for each Admin Node in your StorageGRID system.
...
+
NOTE: Each Admin Node in your StorageGRID system must have an SP connection. This ensures that users can securely sign in to and out of any Admin Node. You can create a single SP connection for all Admin Nodes (recommended) or a different SP connection for each Admin Node.
...
. Verify that the following options are selected for REUSE EXISTING PERSISTENT ACCESS GRANTS FOR GRANT TYPES: 
+
* *IMPLICIT*
* *AUTHORIZATION CODE*
...
. Select *Next*.
. On the Attribute Contract tab, remove any existing attributes that aren't required for the <<data-store-oidc,data store>> you created. Retain only the attributes that you want to pass from Ping Authentication to StorageGRID. 
+
...
. On the Contract Fulfillment tab, enter the following information:
