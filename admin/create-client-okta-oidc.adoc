...
[.lead]
Use Okta to create a single application group for all Admin Nodes for OpenID Connect (OIDC) in your StorageGRID system.
...
+
* Sign-in redirect URI: `\<https://<admin_IP_address>:<port>/api/v4/oidc-callback`
* Sign-out redirect URI: `\<https://<admin_IP_address>:<port>/api/v4/oidc-logged-out`
+
NOTE: If one or more load balancer endpoints are configured for the management interface, add the redirect URIs for each port: `\https://<Admin_IP_address_or_FQDN>:<port>/api/v4/oidc-callback` and `\https://<Admin_IP_address_or_FQDN>:<port>/api/v4/oidc-logged-out`
