...
---
= Configure the StorageGRID internal firewall
:icons: font
:imagesdir: ../media/
...
For more information about using firewall controls, including examples, refer to link:../admin/manage-firewall-controls.html[Manage firewall controls].

For more information about external firewalls and network security, refer to link:../admin/controlling-access-through-firewalls.html[Control access at external firewall].
...
NOTE: The settings on the Privileged address list tab can override the settings on the Untrusted Client Network tab. 

.Steps

. On the Privileged address list tab, enter the address or IP subnet you want to grant access to closed ports. 

. Optionally, select *Add another IP or a subnet in CIDR notation* to add additional privileged clients. 
+
TIP: Add as few addresses as possible to the privileged list.

. Optionally, select *Allow privileged IP addresses to access StorageGRID internal ports*. Refer to link:../network/internal-grid-node-communications.html[StorageGRID internal ports]. 
+
...
. Select *Save*.
+
The new firewall settings are immediately applied and enforced. Existing client connections might fail if load balancer endpoints haven't been configured.

// 2026 MAR 20, SGRIDDOC-229