...
[NOTE]
====
The settings on the Privileged address list tab can override the settings on the Untrusted Client Network tab. 

Add IP addresses or subnets in CIDR notation that can access ports closed for external communication on the Manage external access tab.

Privileged IP addresses and subnets have access to endpoints blocked by untrusted Client Network settings.
====
.Steps
