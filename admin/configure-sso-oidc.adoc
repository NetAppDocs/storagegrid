...
. Use the SSO identity provider's software to configure OIDC for each Admin Node.
...
+
NOTE: You must configure OIDC for each Admin Node in your StorageGRID system. Having an OIDC configuration for each Admin Node ensures that users can securely sign in to and out of any Admin Node.
