...
.Details
For security reasons, when a tenant account is synced from the tenant's source grid to the tenant's destination grid, the password you set for the tenant's local root user isn't synced. Similarly, when a tenant creates local users on its source grid, the local user passwords aren't synced to the destination grid.
...
Before a synced local user can sign in to the tenant's destination grid, the root user for the synced tenant must add a password for the user on the destination grid. For instructions, refer to link:../tenant/manage-users.html[Manage users] in the Tenant Manager.
...
. Select *Retry sync*. If the error has been resolved, the tenant account will now be synced to the other grid.
