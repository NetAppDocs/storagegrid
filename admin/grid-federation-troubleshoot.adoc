...
Before a cloned local user can sign in to the tenant's destination grid, the root user for the cloned tenant must add a password for the user on the destination grid. For instructions, refer to link:../tenant/manage-users.html[Manage users] in the instructions for using the Tenant Manager.
...
+
image:grid-federation-retry-account-clone.png[screenshot showing an error message and the Retry account clone button]
