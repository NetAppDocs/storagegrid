...
summary: "You might need to troubleshoot alerts and errors related to grid federation connections, account sync, and cross-grid replication."
---
...
[.lead]
You might need to troubleshoot alerts and errors related to grid federation connections, account sync, and cross-grid replication.
...
=== Can't sign in to a synced tenant account

.Issue
You can't sign in to a synced tenant account. The error message on the Tenant Manager sign-in page is "Your credentials for this account were invalid. Please try again."

.Details
For security reasons, when a tenant account is synced from the tenant's source grid to the tenant's destination grid, the password you set for the tenant's local root user is not synced. Similarly, when a tenant creates local users on its source grid, the local user passwords aren't synced to the destination grid.
...
Before a synced local user can sign in to the tenant's destination grid, the root user for the synced tenant must add a password for the user on the destination grid. For instructions, refer to link:../tenant/manage-users.html[Manage users] in the instructions for using the Tenant Manager.

=== Tenant created without a sync

.Issue
You see the message "Tenant created without a sync" after creating a new tenant with the *Use grid federation connection* permission. 
...
. Locate the tenant account that failed to be synced.

. Select the tenant name to display the details page.

. Select *Retry account sync*.
+
image::../media/grid-federation-retry-account-clone.png[screenshot showing an error message and the Retry account sync button]
If the error has been resolved, the tenant account will now be synced to the other grid.
...
.Details
For each grid federation connection, the *Last error* column shows the most recent error to occur, if any, when a tenant's data was being replicated to the other grid. This column shows only the last cross-grid replication error to occur; previous errors that might have occurred aren't shown. An error in this column might occur for one of these reasons:

* The source object version wasn't found.
* The source bucket wasn't found.
...
* The source object has S3 Object Lock settings that don't comply with the destination grid's tenant-level retention settings.
* The source object has S3 Object Lock settings, and S3 Object Lock is disabled on the destination bucket. 
...
+
For example, if versioning was suspended on the destination bucket for cross-grid replication, reenable versioning for that bucket. Or, if S3 Object Lock was disabled on the destination bucket for cross-grid replication, enable S3 Object Lock for that bucket.
