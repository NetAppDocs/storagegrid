...
---
= Create relying party trusts in AD FS using OIDC
:icons: font
:imagesdir: ../media/
...
. When you are done, return to StorageGRID and link:../admin/configure-sso-saml.html#test-sso[test all relying party trusts] to confirm they're configured correctly.
...
If the metadata is missing, confirm that the Federation metadata address is correct, or enter the values manually.
. Repeat these steps to configure a relying party trust for all of the Admin Nodes in your StorageGRID system.
. When you are done, return to StorageGRID and link:../admin/configure-sso-saml.html#test-sso[test all relying party trusts] to confirm they are configured correctly.
