...
. Resolve each grid's unique CNAME (for example, `s3.grid-a.example.com`) to the load balancer endpoints for that specific grid.

. For virtual-hosted-style bucket requests (for example, `_bucket_.s3.example.com`), use your organization's routing policy to map bucket names to the appropriate grid's CNAME.
+
