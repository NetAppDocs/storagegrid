...
== Step 2: Set S3global namespace settings
...
CAUTION: You must <<configure-default-region,configure the default region>> before you configure the S3 global namespace settings. When you enable global namespace settings, S3 request will fail if they don't specify the correct region. All S3 requests need to be signed with the grid's default region.
