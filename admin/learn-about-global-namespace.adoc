...
In the <<gns-example,example above>>, the DNS resolver should resolve `bucket123.s3.company.com` to `bucket123.s3.grid3.company.com` (bucket and grid's CNAME). In other words, all requests are sent directly to that grid without additional routing needed.
...
* A client sends a CreateBucket request to `s3.company.com` with `--region us-west-1`.
