...
---
= Manage internal firewall controls in StorageGRID
:icons: font
:imagesdir: ../media/
...
Suppose you want to lock down sensitive ports and the service on that port. You could use the following general steps: 
...
After you save your configuration, the sensitive port and the service on that port is available to hosts on the privileged address list. All other hosts are denied access to the service no matter what interface the request comes from.
