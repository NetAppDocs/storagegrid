...
---
= Access and complete the S3 setup wizard in StorageGRID
:icons: font
:imagesdir: ../media/
...
. If the *FabricPool and S3 setup wizard* banner appears on the dashboard, select the link in the banner. If the banner no longer appears, select the help icon image:icon_help_menu_bar.png[Help icon] from the header bar in the Grid Manager and select *FabricPool and S3 setup wizard*.
...
*Note:* Ports used by other grid services aren't permitted. See 
link:../network/internal-grid-node-communications.html#storagegrid-internal-ports[StorageGRID internal ports].
...
. Define root access for the tenant account, based on whether your StorageGRID system uses link:using-identity-federation.html[identity federation], link:how-sso-works.html[single sign-on (SSO)], or both. 
+
[cols="1a,2a" options="header"]
|===
| Option 
...
** Select the copy icon (image:icon_tenant_copy_url.png[copy icon]) to copy the access key ID and secret access key to the clipboard.
