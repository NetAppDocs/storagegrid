...
A bucket is a container used to store a tenant's objects and object metadata. Although tenants might have many buckets, the wizard helps you to create a  tenant and a bucket in the quickest and easiest way. If you need to add buckets later, you can use the Tenant Manager.

You can create a new tenant for this S3 application to use. For the tenant you create, you can optionally:

* Allow the wizard to create the S3 access keys for the tenant's root user.
* Create a bucket for the new tenant.
