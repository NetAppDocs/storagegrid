...
summary: "You can follow the Configure SSO wizard for SAML and enter sandbox mode to configure and test single sign-on (SSO) before enabling it for all StorageGRID users. After SSO is enabled, you can return to sandbox mode when needed to change or retest the configuration."
---
...
== [[enter-sandbox-mode]]Step 2: Provide relying party trusts, enterprise applications, or SP connections
...
. Follow the steps in link:../admin/creating-enterprise-application-entra-id-saml.html[Create enterprise applications in Entra ID] to create an enterprise application for each Admin Node listed in the table.
...
+
For instructions, go to link:../admin/creating-relying-party-trusts-in-ad-fs-saml.html[Create relying party trusts in AD FS].
...
. Follow the steps in link:../admin/creating-enterprise-application-entra-id-saml.html[Create enterprise applications in Entra ID] to upload the SAML metadata file for each Admin Node into its corresponding Entra ID enterprise application.
...
. link:../admin/creating-sp-connection-ping-saml.html[Create one or more service provider (SP) connections for StorageGRID]. Use the SP connection ID for each Admin Node (shown in the table on the Configure SSO page) and the SAML metadata you downloaded for that Admin Node. 
+
You must create one SP connection for each Admin Node shown in the table.
--
====

== [[test-sso]]Step 4: Test configuration
...
== [[enable-sso-saml]]Step 5: Enable single sign-on
...
| Active directory
| link:creating-relying-party-trusts-in-ad-fs-saml.html[Create an application group for each Admin Node]
| Entra ID
| link:creating-enterprise-application-entra-id-saml.html[Create an application for each Admin Node]
| PingFederate
| link:creating-sp-connection-ping-saml.html[Create a service provider connection for each Admin Node]
|===
