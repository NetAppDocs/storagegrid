...
summary: "Cross-grid replication is the automatic replication of objects between selected S3 buckets in two StorageGRID systems that are connected in a grid federation connection. Account sync is required for cross-grid replication."
---
= Learn about cross-grid replication in StorageGRID
:icons: font
:imagesdir: ../media/

[.lead]
Cross-grid replication is the automatic replication of objects between selected S3 buckets in two StorageGRID systems that are connected in a link:grid-federation-overview.html[grid federation connection]. link:grid-federation-what-is-account-sync.html[Account sync] is required for cross-grid replication.
...
image:grid-federation-cgr-workflow.png[Cross-grid replication workflow]

== Requirements for cross-grid replication

If a tenant account has the *Use grid federation connection* permission to use one or more link:grid-federation-overview.html[grid federation connections], a tenant user with Root access permission can create buckets ("source" and "destination" buckets) in the corresponding tenant accounts on each grid. These buckets:

* Can have different names from each other
* Can have different regions
* Must have versioning enabled
* Must have one of the following S3 Object Lock configurations:
** source: *disabled*, destination: *disabled*
** source: *enabled*, destination: *enabled*
** source: *disabled*, destination: *enabled*

After you've created both buckets, you can configure cross-grid replication for either or both buckets.
...
image:grid-federation-cross-grid-replication-one-direction.png[image showing grid federation connection in one direction]
...
image:grid-federation-cross-grid-replication.png[image showing replication in one direction compared to replication in both directions]
...
image:grid-federation-cross-grid-replication-delete.png[image showing replicate client delete on both grids]
...
CAUTION: You can enable S3 Object Lock on existing buckets, which could lead to a misconfiguration later.

NOTE: When S3 Object Lock is enabled on the source bucket, make sure it's also enabled on the destination bucket. If S3 Object Lock is disabled on the destination bucket, an error occurs during replication. Refer to link:../admin/grid-federation-troubleshoot.html#cross-grid-replication-alerts-and-errors[Cross-grid replication alerts and errors].

When S3 Object Lock on the source bucket is enabled: 

* The objects are locked with retention settings at the destination in this order:
+
. The source object's retention header values for: 
+
...
+
The destination bucket's default retention doesn’t override the retention settings replicated from the source object.
...
When S3 Object Lock on the source bucket is disabled:
...
* The destination object can't set a legal hold status.
