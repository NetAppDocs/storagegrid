...
.. Optionally, select *Sync existing local users and groups* to sync them to the remote grid. If you want, you can stop the cloning in progress or retry cloning if some local users or groups failed to be synced after the last sync operation was completed.
