...
summary: "You can create grid federation connections between StorageGRID systems if you want to sync tenant details and replicate object data."
---
= Create StorageGRID grid federation connections
:icons: font
:imagesdir: ../media/

[.lead]
You can create grid federation connections between StorageGRID systems if you want to sync tenant details and replicate object data.
...
TIP: Optionally, you can link:../admin/grid-federation-automate-connections.html[automate the creation, editing, and maintenance of grid federation connections].

image:grid-federation-create-connection.png[graphic overview of steps to create a connection on two grids]
...
. Enter a connection name, which is a unique name to help you recognize this connection. For example, `Grid 1-Grid 2`.

. If you want to link:../admin/configure-global-namespace.html[configure a global namespace], select *Enable global namespace*.

. Select *Continue*.

. Enter the grid details, as follows:
...
* link:grid-federation-troubleshoot.html[Troubleshoot the connection], including resolving any alerts and errors related to account sync and cross-grid replication.