...
[.lead]
Follow the Configure SSO wizard and enter sandbox mode to configure and test single sign-on (SSO) before enabling it for all StorageGRID users. After you enable SSO, you can return to sandbox mode when needed to change or retest the configuration. 
...
. Select the protocol type, *OIDC* or *SAML*.
. Select the *SSO type* from the drop-down list.
. If you are using SAML and you selected Active Directory as the SSO type, enter the *Federation service name* for the identity provider, exactly as it appears in Active Directory Federation Service (AD FS).
+
...
== [[enter-sandbox-mode]]Step 3: Provide relying party identifier
...
. Follow the steps in link:../admin/creating-enterprise-application-entra-id.html[Create enterprise applications in Entra ID] to create an enterprise application for each Admin Node listed in the table.
...
== Step 4: Configure relying party trusts, enterprise applications, or SP connections
...
. Follow the steps in link:../admin/creating-enterprise-application-entra-id.html[Create enterprise applications in Entra ID] to upload the SAML metadata file for each Admin Node into its corresponding Entra ID enterprise application.
...
== [[test-sso]]Step 5: Test configuration
