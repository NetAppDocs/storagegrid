...
. Complete the fields in the Provide relying party identifier page based on the SSO type you selected.
// start tabbed block
+
[role="tabbed-block"]
====
...
Follow these steps to configure relying party trusts (Active Directory), complete enterprise applications (Entra ID), or configure SP connections (PingFederate).
