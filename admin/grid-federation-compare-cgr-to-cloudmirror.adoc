...
---
= Compare cross-grid replication and CloudMirror replication in StorageGRID
:icons: font
:imagesdir: ../media/
...
NOTE: You can't use CloudMirror on a bucket replicated by cross-grid replication, and vice versa.
...
CloudMirror replication creates an independent copy of an object in an independent S3 infrastructure. This independent copy is not used as a backup, but is often further processed in the cloud.


| How is it set up?
| . Configure a grid federation connection between two grids. 
. Add new tenant accounts, which are  automatically synced to the other grid. 
. Add new tenant groups and users, which are also synced.
