...
---
= Manage StorageGRID grid federation connections
:icons: font
:imagesdir: ../media/
...
NOTE: While the connection is being edited, account sync or cross-grid replication requests will continue to use the existing connection settings. You must edit one connection at a time and upload one verification file to the remote grid for each connection. Edits are applied after they've been uploaded to the remote grids.
...
image:grid_federation_remove_connection.png[steps to remove grid federation connection]
