...
NOTE: While the connection is being edited, account sync or cross-grid replication requests will continue to use the existing connection settings. Any edits you make to a grid are saved locally but aren't used until they have been uploaded to the other grids, saved, and tested. 
