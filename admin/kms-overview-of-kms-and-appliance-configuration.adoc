...
summary: "Before you can use a key management server (KMS) to secure StorageGRID data on appliance nodes, you must set up one or more KMS servers and enable node encryption for the appliance nodes. When these two configuration tasks are complete, the key management process occurs automatically."
---
= StorageGRID key management server and appliance configuration
:icons: font
:imagesdir: ../media/
...
image:kms_configuration_overview.png["KMS configuration workflow, which is described in the following text"]
