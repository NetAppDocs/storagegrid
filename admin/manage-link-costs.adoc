...
---
= Manage link costs in StorageGRID
:icons: font
:imagesdir: ../media/
...
* Link costs are used by the Load Balancer service on Admin Nodes and Gateway Nodes to direct client connections. Refer to link:../admin/managing-load-balancing.html[Considerations for load balancing].

The diagram shows a three-site grid that has link costs configured among sites:

image::../media/link_costs.png[Conceptual diagram for link costs among data centers]
...
. For each site listed in the Site/Link cost column, enter a cost value from 0 to 100 for the other sites.
+
You can't change the link cost if the source is the same as the destination.

. Discard or save changes:
+
* To discard changes, select *Clear changes*.
