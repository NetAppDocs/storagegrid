...
---
= Manage the TLS and SSH policy in StorageGRID
:icons: font
:imagesdir: ../media/
...
+
image:securitysettings_tls_ssh_policies_current.png[TLS and SSH policies]
...
.FIPS strict
--
Use the FIPS strict policy if you require Common Criteria certification and need to use the NetApp Cryptographic Security Module (NCSM) 3.1.2 or NetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1 module for external client connections to load balancer endpoints, Tenant Manager, and Grid Manager. Using this policy might reduce performance.

The NCSM 3.1.2 and NetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1 module are used in the following operations:
...
+
For more information, refer to NIST Cryptographic Algorithm Validation Program link:https://csrc.nist.gov/projects/cryptographic-algorithm-validation-program/details?validation=40426[Certificate #A7816^].
...
+
*Note*: This module has not been submitted for certification.

+
For more information, refer to NIST Cryptographic Algorithm Validation Program link:https://csrc.nist.gov/projects/cryptographic-module-validation-program/entropy-validations/certificate/223[Entropy Certificate #E223^].
...
Optionally, if your StorageGRID has FIPS 140 cryptography requirements, enable the FIPS mode feature to use the NCSM 3.1.2 and NetApp StorageGRID Kernel Crypto API 6.12.63-1+ntap1 module:
...
+
image:securitysettings-custom-security-policy-copy.png[copying an existing policy to create a custom policy]
