...
---
= Download and install RPM or DEB packages on all Linux hosts for StorageGRID
:icons: font
:imagesdir: ../media/
...
. Code signature verification is manual on a Linux node. Optionally, verify the installation archive:
...
.. Follow the steps to link:../swnodes/download-files-verify.html[manually verify the installation files].
...
+
See the steps for installing StorageGRID host services for Linux in link:../swnodes/installing-storagegrid-webscale-host-services.html[Install StorageGRID host services].
+
