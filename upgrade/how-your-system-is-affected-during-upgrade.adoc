...
---
= How StorageGRID upgrades affect your system
:icons: font
:imagesdir: ../media/
...
During the StorageGRID 12.1 upgrade:

* All StorageGRID appliance nodes are automatically upgraded to StorageGRID Appliance Installer firmware version 12.1.
...
*	SGF6112, SG6160, SG110 and SG1100 appliances are automatically upgraded to BIOS firmware version 3A14.QD and BMC firmware version 3.21.07.
...
For example, you might see the *Unable to communicate with node* alert when services are stopped, or you might see the *Cassandra communication error* alert when some nodes have been upgraded to StorageGRID 12.0 or later but other nodes are still running an earlier version of StorageGRID. In general, these alerts will clear when the upgrade completes.

The *ILM placement unachievable* alert might be triggered when Storage Nodes are stopped during the upgrade to StorageGRID 12.0 or later. This alert might persist for 1 day after the upgrade completes.
...
NOTE: Configuration change restrictions are release specific. If you're upgrading to another StorageGRID release, refer to the list of restricted changes in the upgrade instructions for that release.
...
* You should not manage HA groups, VLAN interfaces, or load balancer endpoints while you're upgrading.

* Don't delete any HA groups until the upgrade is complete. Virtual IP addresses in other HA groups might become inaccessible.
