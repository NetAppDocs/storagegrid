...
SSH access remains open during the upgrade. When the upgrade completes, SSH access is closed by default. This enables SSH protection. You can use the *Block SSH"* tab available on the *Grid Manager* > *Security Settings* page to disable SSH protection by opening the SSH port to external access. Refer to <link>. SSH access between grid nodes isn't blocked during the upgrade.
