...
| 11.9
| TBD
| Support for Docker as the container engine for software-only deployments is deprecated. Docker will be replaced with another container engine in a future release. Refer to the link:../install-nodes/software-requirements.html[software requirements for software-based nodes] for a list of Docker versions currently supported.
