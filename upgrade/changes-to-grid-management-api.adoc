...
The following private endpoints have been added to allow you to link:../maintain/convert-to-data-only-node.html[convert one or more combined Storage Nodes] into data-only nodes. If a Storage Node contains the ADC service, the service is automatically moved to another Storage Node on the same site.
