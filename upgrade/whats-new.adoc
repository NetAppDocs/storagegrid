...
Docker and Podman support:: Support for using Docker as the container run-time for software-only deployments will be removed in a future StorageGRID release. Prepare for this change by using Podman for new StorageGRID 11.9 installations. If you're using Docker with an earlier version of StorageGRID, you won't be able to upgrade to future releases until you change to using Podman only.
