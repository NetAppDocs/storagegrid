...
StorageGRID now uses Hyper-Converged Database (HCD) Cassandra for metadata storage. This version of Cassandra uses disk space more efficiently.

HCD is...and helps with...

When you upgrade to StorageGRID 12.0, metadata load per node will decrease by an estimated 20% to 40%. Exact space savings will vary by system. Savings on disk space isn't guaranteed.

Based on the upgrade to HCD, StorageGRID will perform a metadata format update. This process takes one to three days after you upgrade to StorageGRID 12.0. You won't be able to run any other streaming maintenance procedures during this time.

Storage Node recovery procedure changes...
