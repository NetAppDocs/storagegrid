...
summary: 'You can upgrade to StorageGRID 12.1 and apply the latest hotfix for that release at the same time. The StorageGRID upgrade page provides the recommended upgrade path and links directly to the correct download pages.'
---
= Perform a StorageGRID upgrade
:icons: font
:imagesdir: ../media/

[.lead]
You can upgrade to StorageGRID 12.1 and apply the latest hotfix for that release at the same time. The StorageGRID upgrade page provides the recommended upgrade path and links directly to the correct download pages.
...
The update path on the StorageGRID Upgrade page indicates which major versions (for example, 12.1.0) and hotfixes (for example, 12.1.0.1) you must install to get to the latest StorageGRID release. You should install the recommended versions and hotfixes in the order shown.
...
. Select *Browse* to upload the version upgrade file to StorageGRID: `NetApp_StorageGRID_12.1.0_Software_uniqueID.upgrade`
+
...
. Review the details for each reported precheck and resolve any reported errors. See the https://kb.netapp.com/hybrid/StorageGRID/Maintenance/StorageGRID_12.1_software_upgrade_resolution_guide[StorageGRID software upgrade resolution guide^] for the StorageGRID 12.1 release.
+
...
+
NOTE: For a StorageGRID 12.1 upgrade, if you want to approve individual nodes instead of all nodes, the best practice is to upgrade an entire site before moving on to the next site.
+
...
. As soon as the *Enable features* task is complete (up to one hour), you can start using the link:whats-new.html[new features] in the upgraded StorageGRID version.
+
