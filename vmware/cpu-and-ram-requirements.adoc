...
VMware supports one node per virtual host. Ensure that the StorageGRID node does not exceed the physical RAM available. If the hosts aren't dedicated to running StorageGRID (not recommended), be sure to consider the resource requirements of the other applications.
