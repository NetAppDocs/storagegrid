...
VMware supports one node per virtual machine. Ensure that the StorageGRID node does not exceed the physical RAM available. Each VMware host must be dedicated to running StorageGRID.
