...
* You have confirmed that the default rule in the active ILM policy is compliant. Refer to link:creating-default-ilm-rule.html[Create a default ILM rule] for details.
...
After enabling the S3 Object Lock, a grid administrator can also allow tenant users to enable S3 Object Lock on an existing bucket. After a grid administrator enables this setting, they can disable it. For more information, refer to link:requirements-for-s3-object-lock.html#s3-object-lock-existing-buckets[S3 Object Lock requirements].
...
NOTE: The global Compliance setting is deprecated. If you enabled this setting using a previous version of StorageGRID, the S3 Object Lock setting is enabled automatically. You can continue to use StorageGRID to manage the settings of existing compliant buckets; however, you can't create new compliant buckets. For details, refer to https://kb.netapp.com/Advice_and_Troubleshooting/Hybrid_Cloud_Infrastructure/StorageGRID/How_to_manage_legacy_Compliant_buckets_in_StorageGRID_11.5[NetApp Knowledge Base: How to manage legacy Compliant buckets in StorageGRID 11.5^].
...
+
CAUTION: Enabling S3 Object Lock on an existing bucket might cause unintended data migration from a Cloud Storage Pool back to StorageGRID, depending on your ILM rules. You can't stop this migration after it starts.For more information, refer to link:requirements-for-s3-object-lock.html#s3-object-lock-existing-buckets[S3 Object Lock requirements].
 ...
 ** If the default rule is not compliant, an error appears. You must create and activate a new ILM policy that includes a compliant rule as its default rule. Select *OK*. Then, create a new policy, simulate it, and activate it. Refer to link:creating-ilm-policy.html[Create ILM policy] for instructions.
