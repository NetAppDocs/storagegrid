...
A grid administrator can also enable the S3 Object Lock setting for existing buckets to allow tenant users to enable S3 Object Lock on an existing bucket. After enabling this setting, it can be disabled.
...
. Select *Configuration* > *S3 Object Lock*.
+
...
+
The option to allow S3 Object Lock to be enabled on existing buckets appears.

. Optionally, select *Allow S3 Object Lock to be enabled on existing buckets*.
+
CAUTION: Enabling S3 Object Lock on an existing bucket might cause unintended data migration from a Cloud Storage Pool back to StorageGRID, depending on your ILM rules. You cannot stop this migration after it starts.
