...
=== How to automatically remove expired object delete markers
...
=== How to delete a bucket when expired delete markers are preventing deletion

If you are certain that all contents in a bucket are no longer required, use the Tenant Manager to link:../tenant/deleting-s3-bucket-objects.html[to remove all object versions, including delete markers, current versions, and noncurrent versions], from the bucket.

=== Limitations with delete markers created in older versions
