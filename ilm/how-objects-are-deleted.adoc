...
If a client deletes an object version with a version ID from a link:../tenant/manage-branch-buckets.html[branch bucket], StorageGRID hides the existence of the object version, but doesn't create a delete marker, in the branch bucket. If a client deletes an object without a version ID, StorageGRID will create a delete marker in the branch bucket per standard S3 behavior.
