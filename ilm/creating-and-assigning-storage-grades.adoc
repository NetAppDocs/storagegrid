...
---
= Create and assign storage grades in StorageGRID
:icons: font
:imagesdir: ../media/
...
.. For each custom storage grade you want to add, select *Insert* image:icon_nms_insert.gif[insert icon] to add a row.
 
.. Enter a descriptive label.
+
image:editing_storage_grades.gif[Storage Grades page]

.. Select *Apply Changes*.
.. Optionally, if you need to  modify a saved label, select *Edit* image:icon_nms_edit.gif[edit icon] and select *Apply Changes*.
+
NOTE: You can't delete storage grades.

. Assign new storage grades to Storage Nodes:

.. Locate the Storage Node in the LDR list, and select its *Edit* icon image:icon_nms_edit.gif[edit icon].
.. Select the appropriate storage grade from the list.
+
image:assigning_storage_grades_to_storage_nodes.gif[Storage Grades page]
+
