...
When you enable the global S3 Object Lock setting, the per-tenant settings will depend on the StorageGRID release at the time the tenant was created. Review the compliance settings of existing tenants after enabling the global S3 Object Lock setting.
