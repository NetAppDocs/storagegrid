...
---
= Enter details and filters for a StorageGRID ILM rule
:icons: font
:imagesdir: ../media/
...
*Note:* A value of us-east-1 will match objects in buckets created in the us-east-1 region as well as objects in buckets that have no region specified. See link:configuring-regions-optional-and-s3-only.html[Configure S3 regions].
...
image:advanced_filtering_size_between.png[Advanced Filtering example for object size]
...
image:advanced_filtering_multiple_rows.png[Advanced Filtering example for user metadata]