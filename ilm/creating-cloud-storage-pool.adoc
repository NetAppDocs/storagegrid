...
.. For *AWS IAM Roles Anywhere service endpoint URL*, enter the complete URL that StorageGRID will use to obtain credentials from the IAM Roles Anywhere server, including all the required and optional API parameters assigned to your IAM Roles Anywhere account.
.. For *Trust anchor URN*, 
.. For *Profile URN*, 
.. For *Role URN*, 
.. For *Duration*, 
.. For *End-entity certificate*, select *Browse* and upload the PEM-encoded certificate that StorageGRID will use to identify itself to the IAM Roles Anywhere server.
.. For *Client private key*, select *Browse* and upload the PEM-encoded private key for the client certificate.
--
--
