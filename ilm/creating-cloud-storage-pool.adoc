...
.. *End-entity certificate*: The public key, in Privacy-Enhanced Mail (PEM) format, of the X509 certificate signed by the trust anchor. AWS IAM Roles Anywhere uses this key to issue an STS token.
.. *End-entity key*: The private key of the end-entity certificate.
.. *Server CA bundle* (optional): One or more trusted CA certificates, in Privacy-Enhanced Mail (PEM) format, for verifying the IAM Roles Anywhere server. If omitted, the server won't be verified.
--
