...
.. *AWS IAM Roles Anywhere service endpoint URL*: Enter the complete URL for GET requests to obtain credentials from the AWS IAM Anywhere Server API.
...
.. *End-entity certificate*: Select *Browse* and upload one or more CA certificates for verifying the AWS IAM Anywhere Server. Use PEM encoding.
.. *Client private key*: Enter the PEM-encoded private key for the end entity certificate. If the private key is encrypted, use the traditional format. PKCS #8 format is not supported.
--
