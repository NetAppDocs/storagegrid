...
*Note:* To use this option, updates to Last access time must be enabled for the S3 bucket. Refer to link:using-last-access-time-in-ilm-rules.html[Use Last access time in ILM rules].
...
|===
+
NOTE: If you want to create a compliant rule, you must select *Ingest time*. Refer to link:managing-objects-with-s3-object-lock.html[Manage objects with S3 Object Lock].
+
