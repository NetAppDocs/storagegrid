...
---
= S3 Object Lock requirements in StorageGRID
:icons: font
:imagesdir: ../media/
...
When the global S3 Object Lock setting is enabled, you can optionally allow users to enable S3 Object Lock on existing buckets. 

* Enabling the global S3 Object Lock setting allows all S3 tenant accounts to enable S3 Object Lock on existing buckets. 
* When S3 tenant accounts enable S3 Object Lock on an existing bucket, it might cause unintended data migration from a Cloud Storage Pool back to StorageGRID, depending on your ILM rules. You cannot stop this migration after it starts.
