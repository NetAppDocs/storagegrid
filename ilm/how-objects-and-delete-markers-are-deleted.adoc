...
How you configure StorageGRID to delete expired delete markers depends on whether the bucket is intended for long-term use, is being prepared for deletion, or has versioning suspended:
...
* *Prepare bucket for deletion*: If you want to delete a bucket and you are certain that all the contents in the bucket are no longer required, but expired markers are preventing deletion, use the Tenant Manager to link:../tenant/deleting-s3-bucket-objects.html[remove all object versions, including delete markers, current versions, and noncurrent versions], from the bucket.

* *Vresioning suspended*: If you issue a delete-object request without a version ID for an object in a bucket that has versioning suspended, StorageGRID creates a delete marker for the object. This is known as a null-versioned delete marker. In this scenario, you must use `ExpiredObjectDeleteMarker`in a bucket lifecycle to delete expired null-versioned delete markers.

How StorageGRID removes delete markers from a bucket depends on the version:
