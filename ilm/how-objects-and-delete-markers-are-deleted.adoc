...
The response to an object delete request differs depending on whether the request specifies a version ID:

* *No version ID*: The object delete requests without a version ID don't delete the current version of the object and don't free storage space. Instead, these object delete requests create a <<s3-delete-markers,zero-byte delete marker>> as the current version of the object, which makes the previous version of the object "noncurrent." The noncurrent objects are identified using a version ID.
+
If a client deletes an object without a version ID from a branch bucket, StorageGRID creates a delete marker in the branch bucket according to standard S3 behavior.
+
...
* *Includes version ID*: If a client deletes an object version with a version ID from a link:../tenant/what-is-branch-bucket.html[branch bucket], StorageGRID hides the existence of the object version but doesn't create a delete marker in the branch bucket.
...
+
CAUTION: Removing object copies and making freed storage space available is considered permanent and unrecoverable.
...
* *Delete a bucket*: If you want to delete a bucket and you are certain that all the contents in the bucket are no longer required, but expired markers are preventing deletion, use the Tenant Manager to link:../tenant/deleting-s3-bucket-objects.html[remove all object versions, including delete markers, current versions, and noncurrent versions], from the bucket.
