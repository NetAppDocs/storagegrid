...
If you issue a delete-object request without a version ID for an object in a bucket that has versioning suspended enabled, StorageGRID creates a delete marker with a null `versionId` for the object. This replaces and permanently deletes the previous null version because an object can only have one null version.
...
How you configure StorageGRID to delete expired delete markers depends on whether the bucket is intended for long-term use (with or without versioning suspended enabled) or is being prepared for deletion:

* *Automatic cleanup of expired delete markers in an existing bucket*: To automatically remove expired delete markers from a bucket without versioning suspended while retaining object versions, you need to configure a bucket lifecycle rule with the `Expiration` action and the `ExpiredObjectDeleteMarker` request. This action applies only to expired delete markers. When a delete marker expires, it's automatically removed, as shown in the link:example-8-priorities-for-s3-bucket-lifecycle-and-ilm-policy.html#use-bucket-to-lifecycle-remove-expired-delete-markers[example of using a bucket lifecycle rule to remove expired object delete markers].
+
...
+
If you issue a delete-object request without a version ID for an object in a bucket that has versioning suspended enabled, StorageGRID creates a delete marker with a null `versionId` for the object. This replaces and permanently deletes the previous null version because an object can only have one null version. To delete expired null-versioned delete markers, you must use `ExpiredObjectDeleteMarker` in a bucket lifecycle.
