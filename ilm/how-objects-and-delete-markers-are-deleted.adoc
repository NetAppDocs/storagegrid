...
* *Versioning suspended*: If you issue a delete-object request without a version ID for an object in a bucket that has versioning suspended, StorageGRID creates a delete marker for the object. This is known as a null-versioned delete marker. In this scenario, you must use `ExpiredObjectDeleteMarker` in a bucket lifecycle to delete expired null-versioned delete markers.
