...
---
= Create ILM policies in StorageGRID
:icons: font
:imagesdir: ../media/
...
NOTE: If the link:enabling-s3-object-lock.html[global S3 Object Lock setting has been enabled], you must ensure that the ILM policy is compliant with the requirements of buckets that have S3 Object Lock enabled. In this section, follow the instructions that mention having S3 Object Lock enabled.
...
--
 ** The link:enabling-s3-object-lock.html[global S3 Object Lock setting is already enabled] for the StorageGRID system.
...
--
To activate multiple policies, you must create policy tags and assign a policy to each tag. You can create a maximum of 10 policy tags for your grid.

TIP: When multiple policy tags are in use, if tenants frequently reassign policy tags to buckets, grid performance might be impacted. If you have untrusted tenants, consider using only the Default policy tag.
