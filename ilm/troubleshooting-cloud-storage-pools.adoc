...
---
= Troubleshoot Cloud Storage Pools in StorageGRID
:icons: font
:imagesdir: ../media/
...
* When `validateChecksumForDefaultRetention` is set to `true` and default retention is enabled.  
+
To correct this issue, edit the HTTP or HTTPS endpoint connections:
+  
** If you're using HTTP: Cloud Storage Pools with S3 Object Lock enabled aren't supported. Use HTTPS instead.
** If you're using HTTPS: Follow the steps in link:editing-cloud-storage-pool.html[Edit a Cloud Storage Pool] without making any changes. This action triggers the validation of the Cloud Storage Pool configuration that automatically detects and updates the S3 Object Lock flag on a Cloud Storage Pool endpoint configuration.

* When you enable S3 Object Lock with default retention for your StorageGRID S3 bucket after you start using this bucket for a Cloud Storage Pool.
+
To correct this issue, contact technical support to modify the S3 Object Lock settings on the Cloud Storage Pool endpoint.
