...
You might encounter this error when you try to delete a Cloud Storage Pool. This error occurs if the authentication requires an X.509 certificate to ensure the correct external Cloud Storage Pool is validated and the external pool is empty before the Cloud Storage Pool configuration is deleted.
