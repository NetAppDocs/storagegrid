...
* `Cloud Storage Pool Test Failed` for the AMRA or anonymous access authentication type

== Error: Endpoint check failure during new Cloud Storage Pool configuration

You might encounter this error when both the default retention mode and S3 Object Lock are enabled after a Cloud Storage Pool endpoint is created in StorageGRID.

Try correct this issue, follow the steps in link:editing-cloud-storage-pool.html[Edit a Cloud Storage Pool] without making any changes. This triggers the validation of the Cloud Storage Pool configuration to update the S3 Object Lock flag.
