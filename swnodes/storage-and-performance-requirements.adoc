...
---
= Storage and performance requirements for StorageGRID
:icons: font
:imagesdir: ../media/

[.lead]
You must understand the storage requirements for StorageGRID nodes to ensure you have enough space to support the initial configuration and future storage expansion.
...
* *Container pool* -- Performance-tier (10K SAS or SSD) storage for the node containers, which is assigned to the container engine storage driver when you install and configure the container engine on the hosts that are supporting your StorageGRID nodes.
* *System data* -- Performance-tier (10K SAS or SSD) storage for per-node persistent storage of system data and transaction logs, which the StorageGRID host services consume and map into individual nodes.
...
You must use RAID-backed block devices for all storage categories. Non-redundant disks, SSDs, or JBODs aren't supported. You can use shared or local RAID storage for any of the storage categories; however, if you want to use the node migration capability in StorageGRID, you must store both system data and object data on shared storage. For more information, refer to link:node-container-migration-requirements.html[Node container migration requirements].
...
NOTE: In a production deployment, don't run more than one Storage Node on a single physical or virtual host. A dedicated host for each Storage Node provides an isolated failure domain.

You can deploy other types of nodes, such as Admin Nodes or Gateway Nodes, on the same hosts, or you can deploy them on their own dedicated hosts as required.

NOTE: You can't use disk snapshots to restore grid nodes. Instead, refer to the link:../maintain/warnings-and-considerations-for-grid-node-recovery.html[grid node recovery] procedures for each type of node.

== Number of storage volumes for each node

The following table shows the number of storage volumes (LUNs) required for each host and the minimum size required for each LUN, based on which nodes are deployed on that host.

The maximum tested LUN size is 100 TiB.
...
*Note:* A Linux software-based Storage Node and a VMware software-based Storage Node can have 1 to 48 storage volumes. At least 3 storage volumes are recommended.

a|12 TB (4 TB/LUN, minimum)

Maximum tested LUN size: 100 TiB.

Refer to <<storage_req_SN,Storage requirements for Storage Nodes>> for more information.
...
a|200 GB
|===
NOTE: Depending on the audit level configured, the size of user inputs such as S3 object key name, and how much audit log data you need to preserve, you might need to increase the size of the audit log LUN on each Admin Node. Generally, a grid generates approximately 1 KB of audit data per S3 operation, which means that a 200 GB LUN would support 70 million operations per day or 800 operations per second for two to three days.

== Minimum storage space for a host

The following table shows the minimum storage space required for each type of node. You can use this table to determine the minimum amount of storage you must provide to the host in each storage category, based on which nodes are deployed on that host.
...
Suppose you plan to deploy three nodes on the same host or virtual machine: one Storage Node, one Admin Node, and one Gateway Node. You should provide a minimum of nine storage volumes to the host. You need a minimum of 300 GB of performance-tier storage for the node containers, 700 GB of performance-tier storage for system data and transaction logs, and 12 TB of capacity-tier storage for object data.
...
Linux and VMware have the following storage requirements for Storage Nodes:

* A Linux software-based Storage Node can have 1 to 48 storage volumes.
* A VMware software-based Storage Node can have 1 to 48 storage volumes.
...
When installing a grid with metadata-only Storage Nodes, the grid must also contain a minimum number of nodes for object storage. Refer to link:../primer/what-storage-node-is.html#types-of-storage-nodes[Types of Storage Nodes] for more information about metadata-only Storage Nodes.
...
* When configuring different Storage Nodes for a site, use the same setting for volume 0 if possible. If a site contains Storage Nodes of different sizes, the Storage Node with the smallest volume 0 determines the metadata capacity of that site.

For details, go to link:../admin/managing-object-metadata-storage.html[Manage object metadata storage].

// 2026 FEB 27, SGRIDDOC-221