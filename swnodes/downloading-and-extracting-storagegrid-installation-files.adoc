...
. In the *Install StorageGRID* column, select the .tgz or .zip installation archive for your software-based node type: Red Hat Enterprise Linux, Ubuntu or Debian or VMware.
+
NOTE: Use the `.zip` file if you are running Windows on the service laptop.

. Save the installation archive.

. [[download-verification-package]]If you need to verify the installation archive:
