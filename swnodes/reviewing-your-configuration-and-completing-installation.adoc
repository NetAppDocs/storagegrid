...
---
= Review your StorageGRID configuration
:icons: font
:imagesdir: ../media/
...
+
image:11_gmi_installer_summary_page.gif[Grid Manager installer summary page]
...
. Select *Install*.
+
NOTE: If a node is configured to use the Client Network, the default gateway for that node switches from the Grid Network to the Client Network when you select *Install*. If you lose connectivity, you must ensure that you are accessing the primary Admin Node through an accessible subnet. See link:../network/index.html[Networking guidelines] for details.
...
. Select the *I have successfully downloaded and verified the Recovery Package file* checkbox, and select *Next*.
+
...
+
image:12_gmi_installer_status_page.gif[This image is explained by the surrounding text.]
+
