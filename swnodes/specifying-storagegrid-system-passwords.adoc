...
+
image:10_gmi_installer_passwords_page.gif[This image is explained by the surrounding text.]
...
+
NOTE: You are prompted to download the Recovery Package file (`sgws-recovery-package-id-revision.zip`) after you select *Install* on the Summary page. You must link:../maintain/downloading-recovery-package.html[download this file] to complete the installation. The passwords required to access the system are stored in the `Passwords.txt` file, contained in the Recovery Package file.
