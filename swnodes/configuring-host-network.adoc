...
---
= Configure the StorageGRID host network for Linux deployments
:icons: font
:imagesdir: ../media/
...
* You have reviewed the information in link:../network/index.html[Networking guidelines].
...
* If you are using virtual hosts, you have read the <<mac_address_cloning,considerations and recommendations for MAC address cloning>> before configuring the host network.
...
MAC address cloning causes the container to use the MAC address of the host, and the host to use the MAC address of either an address you specify or a randomly generated one.

=== Enabling MAC cloning

In certain environments, security can be enhanced through MAC address cloning because it enables you to use a dedicated virtual NIC for the Admin Network, Grid Network, and Client Network.
...
image::../media/rhel_install_vlan_diag_1.gif[Install VLAN diagram]
...
image::../media/rhel_install_vlan_diag_2.gif[VLAN interfaces]
...
* link:example-etc-network-interfaces.html[Example /etc/network/interfaces for Ubuntu and Debian]

* link:example-etc-sysconfig-network-scripts.html[Example /etc/sysconfig/network-scripts for RHEL]