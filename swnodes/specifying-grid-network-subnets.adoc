...
. Select the plus sign next to the last entry to add an additional network entry. You must specify all subnets for all sites in the Grid Network.
+
* If you have already deployed at least one node, select *Discover Grid Networks subnets* to automatically populate the Grid Network subnet list with the subnets reported by grid nodes that have registered with the Grid Manager.
* You must manually add any subnets for NTP, DNS, LDAP, or other external servers accessed through the Grid Network gateway.
+
image:4_gmi_installer_grid_network_page.gif[Specify subnets]
