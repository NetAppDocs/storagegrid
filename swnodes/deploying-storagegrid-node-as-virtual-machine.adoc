...
---
= Deploy grid nodes as virtual machines for StorageGRID with VMware vSphere
:icons: font
:imagesdir: ../media/
...
+
You might need to remap a port if your enterprise networking policies restrict access to one or more ports that are used by StorageGRID. Refer to link:../network/index.html[Networking guidelines] for the ports used by StorageGRID.
+
