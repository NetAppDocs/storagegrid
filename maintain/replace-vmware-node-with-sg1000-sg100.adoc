...
summary: 'Replace a primary Admin Node running as a VMware node with an SG1000 or SG100 services appliance.'
---
...
+
*IP address*: You can assign the services appliance the same IP address as the failed primary Admin Node (recommended), or you can select a new unused IP address on each network.
...
. On the Home tab, select *Primary Admin (with Load Balancer)* for Node type.
...
* The IP address is the same as the failed primary Admin Node IP address.

. Select *Start Installation*.
+
Allow approximately ten minutes for the installation to reach the *Load StorageGRID installer* step. 
+
...
. Enter the provisioning passphrase and select *Start Recovery*.
+
The Primary Admin Node Recovery page disappears, then reappears. If you select *Start Recovery* again, a `500: Internal Server Error` appears because the recovery has started but can't send the update to the Grid Manager, which is using the new IP address. Resolve this issue by updating the primary Admin Node IP address in the `/etc/hosts` file:
...
----
+
You should see the following error repeatedly: `curl: (7) Failed to connect to <primary_Admin_Node_name> port9999: No route to host`
+
This is the expected behavior. StorageGRID is attempting to send the update to the primary Admin Node using the failed primary Admin Node IP address.
.. Verify that the container is running:
+
`storagegrid node status`
+
`storagegrid node enter <primary_Admin_Node_name>` 
.. In the `/etc/hosts` file inside the container, replace the failed primary Admin Node IP address with the new IP address. 
.. Save the file.
...
... Switch to root: `su -`
... Enter the following command: `change-ip`
