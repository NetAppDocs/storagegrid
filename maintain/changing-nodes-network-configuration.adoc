...
summary: "You can change the network configuration of one or more nodes using the Change IP tool. You can change the configuration of the Grid Network, or add, change, or remove the Admin or Client Networks. You can also change the LAN IP settings for an appliance node BMC."
---
= Change node network configuration in StorageGRID
:icons: font
:imagesdir: ../media/

[.lead]
You can change the network configuration of one or more nodes using the Change IP tool. You can change the configuration of the Grid Network, or add, change, or remove the Admin or Client Networks. You can also change the LAN IP settings for an appliance node BMC.
...
*Linux:* If you are adding a grid node to the Admin Network or Client Network for the first time, and you did not previously configure ADMIN_NETWORK_TARGET or CLIENT_NETWORK_TARGET in the node configuration file, you must do so now. See the StorageGRID link:../swnodes/index.html[installation instructions] for your Linux operating system.
...
+
image:change_ip_tool_main_menu.png[screenshot showing welcome screen of change IP tool]
...
+
image:change_ip_tool_edit_ip_mask_sample_output.png[screenshot described by surrounding text]
...
+
image:change_ip_tool_validate_sample_error_messages.gif[screenshot described by surrounding text]
+
In this example, validation passed.
+
image:change_ip_tool_validate_sample_passed_messages.gif[screenshot described by surrounding text]
...
. If you've allowed external SSH access, link:../admin/manage-external-ssh-access.html[block access] when you're finished changing the node network configuration.

== Temporarily changing the LACP PDU rate

To perform maintenance operations on network components installed in your appliance, such as upgrading NIC firmware, you can check that the current LACP PDU rate setting meets NIC communication timing requirements. You can non-persistently toggle the LACP PDU rate between fast (1 second wait) and slow (30 second wait), if required.
...
You can change the LAN IP settings for an appliance node BMC.

. In Grid Manager go to *Nodes* > *_grid node_* > *BMC settings*.
. In the LAN IP settings section, select an IP assignment type of *Static* or *DHCP*. 
. Enter the MAC address, IPv4 address (CIDR), and Default gateway for the BMC.
. Select *Save*.

You can also change or view the BMC LAN IP settings using the `PUT /bmc-configuration/root-password` or `GET /bmc-configuration/root-password` API.
