...
To perform maintenance operations on network components installed in your appliance, such as upgrading NIC firmware, you can check the current LACP PDU rate setting. You can alos non-persistently toggle the LACP PDU rate between fast and slow, if required.
...
+
`run-each-node --parallel --service ldr --port 8022 '/usr/sbin/set-lacp-rate.sh --lacprate'`

. To temporarily change the LACP PDU rate, enter the following command: 
+
`run-each-node --parallel --service ldr --port 8022 '/usr/sbin/set-lacp-rate.sh --lacprate _<speed>_'`
+
where `_<version-number>_` is `fast` or `slow`.
