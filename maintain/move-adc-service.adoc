...
NOTE: You don't need to manually move the ADC service before link:../maintain/convert-to-data-only-node.html[converting a Storage Node to a data-only node] or decommissioning a Storage Node. These procedures automatically move the ADC service when required. See link:../maintain/understanding-adc-service-quorum.html[What is the ADC quorum?] for details about the requirements.
