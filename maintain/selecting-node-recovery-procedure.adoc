...
NOTE: If a server that is hosting more than one grid node fails, you can recover the nodes in any order. However, if the failed server is hosting the primary Admin Node, you must recover that node first, unless there is a non-primary Admin Node available. Recovering the primary Admin Node first prevents other node recoveries from halting as they wait to contact the primary Admin Node. If a non-primary Admin Node is available, use it to recover other nodes so those recoveries can proceed without contacting the primary Admin Node.
