...
If you need to decommission a Storage Node, you can continue to meet the ADC quorum requirement by link:../upgrade/changes-to-grid-management-api.html#new-private-endpoints-for-move-adc[Moving the ADC service to another Storage Node on the same site] or adding a new Storage Node in an link:../expand/index.html[expansion] and specifying that it should have an ADC service. Then, you can decommission the existing Storage Node without affecting the quorum.