...
+
NOTE: To configure a relying party trust, see link:../admin/configure-sso-saml.html[Configure single sign-on]. To access the default server certificate, log in to the command shell of the Admin Node. Go to the `/var/local/mgmt-api` directory, and select the `server.crt` file.
