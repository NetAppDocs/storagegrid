...
---
= Decommission disconnected StorageGRID nodes
:icons: font
:imagesdir: ../media/
...
You can identify disconnected nodes by looking for the blue Unknown icon image:icon_alarm_blue_unknown.png[Icon Unknown] or the gray Administratively down icon image:icon_alarm_gray_administratively_down.png[Icon Administratively down] in the *Health* column.
...
. Review the list of nodes, and select *OK*.
+
...
. As soon as the new recovery package is available, select the link or select *Maintenance* > *System* > *Recovery package* to access the recovery package page. Then, download the `.zip` file.
+
...
. If you're decommissioning a Storage Node, monitor the status of the *replicated data* and *erasure-coded (EC) data* repair jobs that are automatically started during the decommissioning process.
+
[role="tabbed-block"]
====
.Replicated data
