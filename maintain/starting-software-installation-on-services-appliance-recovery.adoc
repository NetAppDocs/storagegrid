...
---
= Start StorageGRID software installation on a services appliance
:icons: font
:imagesdir: ../media/
...
* If you are installing a Gateway Node or a non-primary Admin Node, you know the IP address for either of the following Admin Nodes:
** The primary Admin Node for the StorageGRID grid
** A non-primary Admin Node, if one is available and if it's being used to recover the node
* If you are recovering a node (including the Primary Admin Node) by using a non-primary Admin Node, you can specify that Admin Node's IP address.
...
* For a primary Admin Node, you specify the name of the node and then upload the appropriate software packages (if required). If you are recovering the primary Admin Node and you have a non-primary Admin Node in your grid, you have the option of either recovering the node using a non-primary Admin Node and specifying the IP of that Admin Node or you can install or recover the node using your current configuration. 
...
. To install a primary Admin Node:
 ...
 .. In the *Node name* field, enter the same name that was used for the node you are recovering, and select *Save*.
 .. Select an appropriate mode from the two that are available in the Installation Mode section. You can select either the Install a node or recover this node using current configuration method or the Recover this node using a non-primary Admin Node method.
+
...
Install a node or recover this node using the current configuration.

. In the Installation section, check the software version listed under Current state. If the version of software that is ready to install is correct, skip ahead to the <<installation_section_step,Installation step>>.
. If you need to upload a different version of software, under the *Advanced* menu, select *Upload StorageGRID software*. The Upload StorageGRID Software page appears.
. Select *Browse* to upload the *Software package* and *Checksum file* for StorageGRID software. The files are automatically uploaded after you select them.
. Select *Home* to return to the StorageGRID Appliance Installer Home page.

--
.Non-primary Admin Node method
--

Recover this node using a non-primary Admin Node.

. In the non-primary Admin Node IP field, enter the grid network IP address of the non-primary Admin Node you want to use for the recovery, and select *Save*.
...
The StorageGRID Appliance Installer can discover this IP address automatically, assuming the primary Admin Node, or at least one other grid node with ADMIN_IP configured, is present on the same subnet.
 ...
 .. If this IP address isn't shown or you need to change it, specify the address:
...
+
If the *Start Installation* button isn't enabled, you might need to change the network configuration or port settings. Refer to the maintenance instructions for your appliance.
...
+
NOTE: If you need to access the Monitor installation page manually, select *Monitor installation* from the menu bar.
// 2026-04-29,SGWS-34291