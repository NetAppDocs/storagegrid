...
If you recover a node from a non-primary Admin Node, ensure you set the ADMIN_IP to the grid network IP of the non-primary Admin Node, instead of the primary Admin Node. This means that the node registers with the non-primary Admin Node instead of the primary Admin Node. To view the rules for admin network keys see link:../swnodes/creating-node-configuration-files.html#contents-of-a-node-configuration-file.html[Create StorageGRID node configuration files for Linux deployments].
