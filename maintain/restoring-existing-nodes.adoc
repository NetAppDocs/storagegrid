...
---
= Restore StorageGRID nodes to the host
:icons: font
:imagesdir: ../media/
...
If you recover a node from a non-primary Admin Node, set ADMIN_IP to the Grid Network IP address of the _non-primary Admin Node_, so the recovered node registers with the non-primary Admin Node instead of the primary Admin Node. To view the rules for Admin Network keys, refer to link:../swnodes/creating-node-configuration-files.html#contents-of-a-node-configuration-file.html[Create StorageGRID node configuration files for Linux deployments].
...
See the link:../swnodes/index.html[Linux installation instructions] for more information about the location of the `/var/local` volume for a node.
...
. For grid nodes that don't have a `/var/local` volume, re-create the node's configuration file to restore it to the host. For instructions, see link:../swnodes/creating-node-configuration-files.html[Create node configuration files].
+
...
Determine the appropriate size and storage category using the tables for your Linux operating system. See link:../swnodes/storage-and-performance-requirements.html[Storage and performance requirements].

Review the recommendations for link:../swnodes/configuring-host-storage.html[configuring host storage] before proceeding with the block device replacement.
