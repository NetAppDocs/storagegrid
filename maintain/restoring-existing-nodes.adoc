...
See the link:../install-nodes/index.html[Linux installation instructions] for more information about the location of the `/var/local` volume for a node.
...
. For grid nodes that don't have a `/var/local` volume, re-create the node's configuration file to restore it to the host. For instructions, see link:../install-nodes/creating-node-configuration-files.html[Create node configuration files].
+
...
Determine the appropriate size and storage category using the tables for your Linux operating system. See link:../install-nodes/storage-and-performance-requirements.html[Storage and performance requirements].

Review the recommendations for link:../install-nodes/configuring-host-storage.html[configuring host storage] before proceeding with the block device replacement.
