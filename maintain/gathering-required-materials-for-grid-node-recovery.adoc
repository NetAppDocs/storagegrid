...
---
= Gather required materials for StorageGRID grid node recovery
:icons: font
:imagesdir: ../media/
...
When logged in to any Admin Node, use the Grid Manager to download a recovery package. Select *Maintenance* > *System* > *Recovery package*.
...
. Skip to the appropriate section for your platform to choose the files you need, based on your platform and which grid nodes you need to recover.
+
The paths listed in the step for each platform are relative to the top-level directory installed by the archive file.

. If you are recovering a link:../swnodes/index.html[software-based node], select the appropriate files:
+
[role="tabbed-block"]
====
