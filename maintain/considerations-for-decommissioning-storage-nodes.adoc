...
---
= Storage Node decommission considerations in StorageGRID
:icons: font
:imagesdir: ../media/
...
* The system must, at all times, include enough Storage Nodes to satisfy operational requirements, including the link:understanding-adc-service-quorum.html[ADC quorum] and the active link:reviewing-ilm-policy-and-storage-configuration.html[ILM policy]. If a decommission would bring a site's ADC count below quorum, StorageGRID automatically moves the ADC service to available non-ADC Storage Nodes on the same site, maintaining ADC quorum. If the ADC service can't be automatically moved, you might need to link:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.html[add a new Storage Node in an expansion operation] before you can decommission an existing Storage Node.
+
