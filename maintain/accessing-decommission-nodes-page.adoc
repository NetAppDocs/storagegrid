...
| link:../upgrade/changes-to-grid-management-api.html#new-private-endpoints-for-move-adc[Move the ADC service] or perform an expansion by adding a new Storage Node to the site and specifying that it should have an ADC service. Refer to link:understanding-adc-service-quorum.html[ADC quorum].
