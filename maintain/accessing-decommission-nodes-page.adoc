...
| *Storage Nodes only* (combined or metadata-only Storage Nodes). You can't decommission a Storage Node if insufficient nodes would remain at the site to support ADC quorum requirements and the ADC service can't be automatically moved. The automatic move requires more than two ADC Storage Nodes online at the site and available non-ADC Storage Nodes on the same site.
