...
| Ensure that the conditions for automatic ADC service movement are met, or link:../maintain/move-adc-service.html[manually move the ADC service] to another Storage Node on the same site. You can also perform an expansion by adding a new Storage Node to the site and specifying that it should have an ADC service. Refer to link:understanding-adc-service-quorum.html[ADC quorum].
