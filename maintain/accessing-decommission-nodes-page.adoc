...
---
= Access Decommission Nodes page in StorageGRID
:icons: font
:imagesdir: ../media/
...
+
In this example, a green check mark in the Decommission Possible column indicates that you can decommission the API Gateway Node and the data-only Storage Node. The other nodes can't be decommissioned for various reasons. For example, the primary Admin Node can't be decommissioned because decommissioning this node type isn't supported. The metadata-only Storage Node can't be decommissioned because the site would no longer meet the link:../maintain/understanding-adc-service-quorum.html[ADC quorum].
+
image:decommission_nodes_page.png[Decommission Nodes page]
 ...
 ** image:icon_alarm_gray_administratively_down.png[gray questionmark icon] (gray): Administratively Down
 ** image:icon_alarm_blue_unknown.png[blue question mark icon] (blue): Unknown
...
| No, site _x_ needs at least 3 Storage Nodes with ADC services or available non-ADC nodes to move ADC services to. ADC node decommission isn't available if any nodes are offline.
| *Storage Nodes only* (combined or metadata-only Storage Nodes). You can't decommission a Storage Node if insufficient nodes would remain at the site to support ADC quorum requirements and the ADC service can't be automatically moved. The automatic move requires more than two ADC Storage Nodes online across the grid and available non-ADC Storage Nodes on the same site.
| Ensure that the conditions for automatic ADC service movement are met (refer to link:../maintain/understanding-adc-service-quorum.html[ADC quorum]), or link:../maintain/move-adc-service.html[manually move the ADC service] to another Storage Node on the same site. You can also perform an expansion by adding a new Storage Node to the site and specifying that it should have an ADC service. Refer to link:../expand/adding-grid-nodes-to-existing-site-or-adding-new-site.html[Workflow summary for adding grid nodes to StorageGRID].
