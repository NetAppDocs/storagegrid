...
---
= Configure a replacement primary Admin Node in StorageGRID
:icons: font
:imagesdir: ../media/
...
. Select *Recover a failed primary Admin Node*.
+
image:install_or_recover_primary_admin_node.png[Install or recover Primary Admin Node]

. Upload the most recent backup of the recovery package:
 .. Select *Browse*.
 .. Locate the most recent recovery package file for your StorageGRID system, and select *Open*.
...
+
NOTE: To configure a relying party trust, refer to set up SSO using link:../admin/learn-about-setting-up-sso-oidc.html[OIDC] or link:../admin/learn-about-setting-up-sso-saml.html[SAML]. To access the default server certificate, log in to the command shell of the Admin Node. Go to the `/var/local/mgmt-api` directory, and select the `server.crt` file.
