...
+
NOTE: When recovering an appliance Storage Node, reinstall it with the same storage type (Combined, Metadata-only, or Data-only) as the original appliance. If you specify a different storage type the recovery will fail and require reinstallation of the appliance with the correct storage type specified.
