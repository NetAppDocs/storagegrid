...
summary: "If the automatic procedure fails to remount preserved storage volumes and reformat failed storage volumes, you must manually run two scripts to complete those operations. The first script remounts volumes that are properly formatted as StorageGRID storage volumes. The second script reformats any unmounted volumes, rebuilds the Cassandra database, if needed, and starts services."
---
= Remount and reformat StorageGRID appliance storage volumes (manual steps)
:icons: font
:imagesdir: ../media/

[.lead]
If the automatic procedure fails to remount preserved storage volumes and reformat failed storage volumes, you must manually run two scripts to complete those operations. The first script remounts volumes that are properly formatted as StorageGRID storage volumes. The second script reformats any unmounted volumes, rebuilds the Cassandra database, if needed, and starts services.

During a Storage Node recovery, if a "Waiting for manual steps" message appears in the Grid Manager, follow these steps to remount and reformat storage volumes. If you don't see this message, the remount and reformat operations have completed automatically, and you can skip _this_ procedure and link:../maintain/restoring-object-data-to-storage-volume-for-appliance.html[restore object data].
...
CAUTION: Contact technical support if more than one Storage Node is offline. Don't run the `sn-recovery-postinstall.sh` script.
...
+
image:recovering_cassandra.png[screenshot showing recovery progress in Grid Management Interface]
...
+
The script asks if you want to use the Grid Manager volume restoration process to restore object data.

* In most cases, you should link:../maintain/restoring-object-data-to-storage-volume-for-appliance.html[restore object data using Grid Manager]. Answer `y` to use the Grid Manager.

* In rare cases, such as when you know that the replacement node has fewer volumes available for object storage than the original node, contact technical support to restore object data manually. If one of these cases applies, answer `n` and press any key to return to the command line.
