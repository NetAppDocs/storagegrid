...
---
= Recover from StorageGRID Storage Node failures
:icons: font
:imagesdir: ../media/
...
| If more than one Storage Node has failed, technical support must assist with recovery to prevent Cassandra database inconsistencies that could lead to data loss. A site recovery procedure might be required.

Technical support can determine when it's safe to begin recovery of a second Storage Node.

*Note*: If more than one Storage Node that contains the ADC service fails at a site, any pending platform service requests for that site will be lost.
...
| link:reviewing-warnings-about-storage-volume-recovery.html[Recover from storage volume failure where system drive is intact]
