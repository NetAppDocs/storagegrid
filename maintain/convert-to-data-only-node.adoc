...
sidebar: sidebar
keywords: convert node, data-only node
...
.About this task
Use this procedure to convert one or more combined Storage Nodes that don't contain the ADC service into data-only nodes. The Storage Nodes must be on the same site.
