...
summary: 'To increase performance, you can convert combined Storage Nodes to data-only Storage Nodes.'
---
...
[.lead]
To increase performance, you can convert combined Storage Nodes to data-only Storage Nodes.
...
* If the Storage Node contains the ADC service, more than two ADC Storage Nodes are online at the site, and non-ADC Storage Nodes are available on the same site to host the ADC service.
...
[CAUTION]
====
* If the metadata that migrates to other nodes on the same site exceeds the size of the link:../admin/managing-object-metadata-storage.html[metadata reserved space], the grid will become unstable.
