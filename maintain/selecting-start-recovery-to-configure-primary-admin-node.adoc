...
summary: 'Select Start Recovery to configure the primary Admin Node during primary Admin Node recovery.'
---
= Select Start Recovery to configure primary Admin Node
:icons: font
:imagesdir: ../media/

[.lead]
After replacing a primary Admin Node, you must select Start Recovery in the Grid Manager to configure the new node as a replacement for the failed node.
...
NOTE: You can also use the following procedure to recover a failed primary Admin Node from a non-primary Admin Node.
...
+
NOTE: While the recovery procedure is running, you can select *Reset* to start a new recovery. A dialog box appears, indicating that the node will be left in an indeterminate state if you reset the procedure.
+
