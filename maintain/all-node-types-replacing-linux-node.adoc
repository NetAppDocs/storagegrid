...
If more than one grid node is hosted on a physical or virtual Linux host, you can recover the grid nodes in any order. However, recovering a primary Admin Node first, if present, prevents the recovery of other grid nodes from stalling as they try to contact the primary Admin Node to register for recovery. If you are unable to recover the primary Admin Node first, and there is a non-primary Admin Node available, you can use it to recover nodes in any order.
