...
---
= Replace a Linux node in StorageGRID
:icons: font
:imagesdir: ../media/
...
If more than one grid node is hosted on a physical or virtual Linux host, you can recover the grid nodes in any order. However, if the primary Admin Node is present, you must recover it first to prevent stalling the recovery of other grid nodes as they try to contact the primary Admin Node. If you can't recover the primary Admin Node first and a non-primary Admin Node is available, you can use a non-primary Admin Node to recover nodes in any order.
// 2026-04-01,SGWS-34291
