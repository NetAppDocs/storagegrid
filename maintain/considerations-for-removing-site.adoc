...
---
= Site decommission considerations in StorageGRID
:icons: font
:imagesdir: ../media/
...
NOTE: If a site contains a mixture of connected (image:icon_alert_green_checkmark.png[Icon Alert Green check mark]) and disconnected nodes (image:icon_alarm_gray_administratively_down.png[Icon Alarm Gray Administratively Down] or image:icon_alarm_blue_unknown.png[Icon Alarm Blue Unknown]), you must bring all offline nodes back online.
...
* You can't decommission a site if it contains a mixture of connected (image:icon_alert_green_checkmark.png[Icon Alert Green check mark]) and disconnected (image:icon_alarm_blue_unknown.png[Icon Alarm Blue Unknown] or image:icon_alarm_gray_administratively_down.png[Icon Alarm Gray Administratively Down]) nodes.
* You can't decommission a site if any node at any other site is disconnected (image:icon_alarm_blue_unknown.png[Icon Alarm Blue Unknown] or image:icon_alarm_gray_administratively_down.png[Icon Alarm Gray Administratively Down]).
...
* All nodes in your StorageGRID system must have a Connection State of *Connected* (image:icon_alert_green_checkmark.png[Icon Alert Green check mark]); however, the nodes can have active alerts.
+
 ...
 ** *Unknown* (image:icon_alarm_blue_unknown.png[Icon Alarm Blue Unknown]): For an unknown reason, a node is disconnected or services on the node are unexpectedly down. For example, a service on the node might be stopped, or the node might have lost its network connection because of a power failure or unexpected outage.
 ** *Administratively Down* (image:icon_alarm_gray_administratively_down.png[Icon Alarm Gray Administratively Down]): The node is not connected to the grid for an expected reason. For example, the node or services on the node have been gracefully shut down.
* All nodes at all other sites must have a Connection State of *Connected* (image:icon_alert_green_checkmark.png[Icon Alert Green check mark]); however, these other nodes can have active alerts.
