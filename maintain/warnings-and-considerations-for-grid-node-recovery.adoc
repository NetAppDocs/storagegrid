...
---
= Warnings and considerations for StorageGRID node recovery
:icons: font
:imagesdir: ../media/
...
* If you are recovering a grid node other than the primary Admin Node, connectivity must exist between the grid node being recovered and the Admin Node managing the recovery procedure.
...
If a server that is hosting more than one grid node fails, you can recover the nodes in any order. However, if the failed server is hosting the primary Admin Node and no non-primary Admin Nodes exist, you must recover the primary Admin Node first.
