...
* If you are recovering a grid node other than the primary Admin Node, connectivity must exist between the grid node being recovered and the primary Admin Node managing the recovery procedure. This requirement doesn't apply when recovering the primary Admin Node, because it manages its own recovery.
