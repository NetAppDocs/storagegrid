...
* If you are recovering a grid node other than the primary Admin Node, connectivity must exist between the grid node being recovered and the primary Admin Node managing the recovery procedure. This requirement does not apply when recovering the primary Admin Node, because it manages its own recovery.
...
If a server that is hosting more than one grid node fails, you can recover the nodes in any order. However, if the failed server is hosting the primary Admin Node, you must recover that node first.
