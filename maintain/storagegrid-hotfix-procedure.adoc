...
After a hotfix has been applied to your grid, the primary Admin Node or a non-primary Admin Node automatically installs the same hotfix version to any nodes restored by recovery operations or added in an expansion. And you also have the option of recovering any node from a non-primary Admin Node.
