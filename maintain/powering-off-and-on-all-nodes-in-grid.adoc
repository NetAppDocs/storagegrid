...
---
= Power off and on all StorageGRID nodes
:icons: font
:imagesdir: ../med
...
When you power off all nodes in a site or grid, you can't access ingested objects while the Storage Nodes are offline.
...
Stop services on Admin Nodes and Gateway Nodes first, then stop services on Storage Nodes.
...
NOTE: If a host includes multiple grid nodes, the nodes come back online automatically when you power on the host.

.Steps

. Power on the hosts for the primary Admin Node and any non-primary Admin Nodes.
+
NOTE: You can't log in to the Admin Nodes until the Storage Nodes have been restarted.
