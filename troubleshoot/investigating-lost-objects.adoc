...
If the object isn't found, <<reset-lost-obj-count,reset the Lost object count>> to clear the alert. The lack of an object indicates that the object was intentionally deleted.
...
If the objects exist, <<reset-lost-obj-count,reset the Lost object count>> to clear the alert.
...
. [[reset-lost-obj-count]]After you address lost object issues, reset the Lost object count to ensure alerts aren't false positives:
.. Select *NODES*.
.. Select *_Storage Node_* > *Tasks*.
.. In the Objects potentially lost section, select *Reset*.
