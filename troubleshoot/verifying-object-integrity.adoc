...
---
= Verify the integrity of object data in StorageGRID
:icons: font
:imagesdir: ../media/
...
If background verification can't replace a corrupted object because it can't locate another copy, the *Objects potentially lost* alert is triggered.
...
*	If Missing object copies detected is greater than zero and the *Objects potentially lost* alert has not been triggered, then all missing copies were repaired by the system. Verify that any hardware issues have been corrected to prevent future damage to object copies.
* If Missing object copies detected is greater than zero and the *Objects potentially lost* alert has been triggered, then data integrity could be affected. Contact technical support.
* You can investigate potentially lost object copies by using grep to extract the LLST audit messages: `grep LLST audit_file_name`.
+
This procedure is similar to the one for link:../troubleshoot/investigating-potentially-lost-objects.html[investigating potentially lost objects], although for object copies you search for `LLST` instead of `OLST`.
