...
== Node network connectivity error alert

*Node network connectivity error* alerts can be caused by connectivity issues between StorageGRID and your network hardware. This alert clears on its own after the underlying problem is addressed.

.About this task
*Node network connectivity error* alerts can be caused by the following issues with networking hardware that connects to StorageGRID:
...
NOTE: These steps are applicable only for *Node network connectivity error* alerts caused by FEC mismatch on StorageGRID appliances.
