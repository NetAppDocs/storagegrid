...
sidebar: sidebar
keywords: s3 access key, secret key id, account sync, tenant manager api
summary: "If your tenant account has the Use grid federation connection permission, you can use the Tenant Management API to manually sync S3 access keys from the tenant on the source grid to the tenant on the destination grid."
---
= Sync S3 access keys using the API
:icons: font
:imagesdir: ../media/

[.lead]
If your tenant account has the *Use grid federation connection* permission, you can use the Tenant Management API to manually sync S3 access keys from the tenant on the source grid to the tenant on the destination grid.
...
* If you are syncing access keys for a local user, the user already exists on both grids.
+
NOTE: When you sync S3 access keys for a federated user, both the user and the S3 access keys are added to the destination tenant.

== Sync your own access keys

You can sync your own access keys if you need to access the same buckets on both grids.
...
. Confirm that the server response code is *204*, indicating that the key was successfully synced to the destination grid. 

== Sync another user's access keys

You can sync another user's access keys if they need to access the same buckets on both grids.
...
. Obtain the user ID. You will need this value to sync the other user's access keys.
...
.. Find the user whose keys you want to sync, and copy the number in the *id* field. 
