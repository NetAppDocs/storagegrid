...
---
= Learn about StorageGRID branch buckets
:icons: font
:imagesdir: ../media/
...
* You're saving data in a versioned bucket. There was an accidental vulnerability that caused many unwanted objects to be ingested between time _T1_ and _T2_. You can create a branch bucket for the _Exclude_ time values, _T1_ and _T2_, and redirect client operations to that branch bucket. Then, only objects ingested before _T1_ and after _T2_ are exposed to clients.
...
Also refer to link:../ilm/how-objects-and-delete-markers-are-deleted.html#delete-s3-versioned-objects[How S3 versioned objects are deleted].
