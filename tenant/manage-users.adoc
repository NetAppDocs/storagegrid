...
---
= Manage StorageGRID tenant users
:icons: font
:imagesdir: ../media/

[.lead]
You can create local users and assign them to local groups to determine which features these users can access. You can also import federated users. The Tenant Manager includes one predefined local user, named _root_. Although you can add and remove local users, you can't remove the root user.

NOTE: If single sign-on (SSO) is enabled for your StorageGRID system, local users won't be able to sign in to the Tenant Manager or the Tenant Management API, although they can use client applications to access the tenant's resources, based on group permissions.
...
* If your tenant account has the *Use grid federation connection* permission, you have reviewed the workflow and considerations for link:grid-federation-account-sync.html[syncing tenant groups and users], and you are signed in to the tenant's source grid.
...
. Select *Create user*.
+
If your tenant account has the *Use grid federation connection* permission and you are on the tenant's source grid, the new local user is synced to the tenant's destination grid. *Succeeded* appears as the *Sync status* in the Overview section of the user's detail page.
...
.. In the Overview section, change the full name by selecting the name or the edit icon image:icon_edit_tm.png[Edit icon].
+
...
NOTE: If your tenant account has the *Use grid federation connection* permission and you duplicate a user from the tenant's source grid, the duplicated user will be synced to the tenant's destination grid.
...
== [[sync-users]]Retry user sync

If any user failed to sync, "Failed" appears in the Sync status column for the user. A banner appears above the table to indicate that one or more users failed to sync.

.Steps

To retry syncing:

. On the banner, select *Retry sync*. This operation retries syncing for all users that failed to sync.
. To retry syncing an individual user, select the user name. On the user details page, select *Retry sync*.
. View the status of the sync operation from the details page of each user you're syncing.

If the retry is successful, "Succeeded" appears in the Sync status column for each user that was successfully synced, and the banner disappears.

For additional information, see link:grid-federation-account-sync.html[Sync tenant groups and users].
...
NOTE: If your tenant account has the *Use grid federation connection* permission and you delete a local user, StorageGRID will delete the corresponding user on the other grids in the connection, unless that user is a federated user on a target grid. You must use the federated identity source to delete federated users.
