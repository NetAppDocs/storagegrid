...
NOTE: The size of all objects belonging to this tenant includes incomplete and in-progress multipart uploads. The size does not include the additional space required for replicated and erasure-coded copies. The Space used values are estimates. These estimates are affected by the timing of ingests, network connectivity, and node status.
