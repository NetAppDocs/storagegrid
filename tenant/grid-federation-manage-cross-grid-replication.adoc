...
The workflow diagram summarizes the steps that you perform to configure cross-grid replication between buckets on two grids. These steps are described in more detail below.
...
* The tenant user you are signing in as already exists on both grids and belongs to a user group that has the link:tenant-management-permissions.html[Root access permission].
* If you are signing in to the tenant's destination grid as a local user, the root user for the tenant account has set a password for your user account on that grid.
...
+
After a few moments, a success message appears. Objects added to this bucket will be automatically replicated to the other grid. *Cross-grid replication* is shown as an enabled feature on the bucket details page.
