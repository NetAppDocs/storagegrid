...
+
A capacity limit is the maximum number of bytes available for this buckets's objects. This value represents a logical amount (object size), not a physical amount (size on disk). If you don't set a capacity limit, the bucket's capacity is limited only by the tenant's quota if one is set.
