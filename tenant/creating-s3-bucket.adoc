...
---
= Create a StorageGRID S3 bucket
:icons: font
:imagesdir: ../media/
...
+
NOTE: Permissions to set or modify S3 Object Lock properties of buckets or objects can be granted by link:../s3/use-access-policies.html[bucket policy or group policy].
...
* If multiple tenants will each have up to 5,000 buckets, each Storage Node in the grid has a minimum of 64 GB of RAM.
+
NOTE: The StorageGRID system supports a maximum of 20,000 buckets for each S3 tenant account. Each grid can have a maximum of 100,000 buckets, including link:../tenant/what-is-branch-bucket.html[branch buckets]. There's also a limit to link:../tenant/manage-branch-buckets.html[branch bucket depth].
...
For more information, refer to the https://docs.aws.amazon.com/AmazonS3/latest/userguide/bucketnamingrules.html[Amazon Web Services (AWS) documentation on bucket naming rules^].
...
Your StorageGRID administrator manages the available regions. A bucket's region can affect the data-protection policy applied to objects. By default, all buckets are created in the `us-east-1` region. If the default region is configured to a region other than `us-east-1`, this other region is initially selected in the drop-down.
...
+
You must enable object versioning if:
+
* The bucket will be used for cross-grid replication.
* You want to create a link:../tenant/what-is-branch-bucket.html[branch bucket] from this bucket.
...
. [[capacity-limit]]Optionally, select *Enable capacity limit*, enter a value, and select the capacity unit.
+
...
. [[object-count-limit]]Optionally, select *Enable object count limit*.
+
Object count limit is the maximum number of objects this bucket can contain. This value represents a logical amount (object count). If no limit is set, the object count is unlimited.
...
You can also link:../tenant/manage-branch-buckets.html[create branch buckets] as needed.
