...
+
NOTE: Each grid can have a maximum of 100,000 buckets, including link:../tenant/what-is-branch-bucket.html[branch buckets].
...
+
Capacity limit is the maximum capacity available for this bucket's objects. This value represents a logical amount (object size), not a physical amount (size on disk). The maximum supported object size is 100 TiB.
+
...
+
Object count limit is the maximum number of objects this bucket can contain. This value represents a logical amount (object count). The maximum supported object count is 48. 
+
If no limit is set, the object count is unlimited.
. Select *Create bucket*.
+
