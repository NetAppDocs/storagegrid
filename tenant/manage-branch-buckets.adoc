...
* A branch bucket adheres to existing link:../s3/bucket-and-group-access-policies.html[access policies] and link:../ilm/access-create-ilm-rule-wizard.html[ILM rules] for the corresponding base bucket.
...
*Note*: You can set the branch bucket type to read-only only if the branch bucket is empty. If the type for an existing branch bucket is set to read-write and you haven't written to the it, you can change the type to read-only.
|===
