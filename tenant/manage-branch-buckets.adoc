...
summary: 'A branch bucket provides access to objects in a bucket as they existed at a certain time, provided they still exist in the bucket. Use the Tenant Manager to create and view branch buckets.'
---
...
[.lead]
A branch bucket provides access to objects in a bucket as they existed at a certain time, provided they still exist in the bucket. Use the Tenant Manager to create and view branch buckets.
...
* S3 Object Lock
* Cross-grid replication for base buckets without branch buckets
* Bucket policies for versioned buckets to clean up old object versions
...
| Branch bucket type
|
* *Read-write*: You can add or delete objects or object versions in the branch bucket, and you can modify bucket options and bucket access. For example, you can add new objects or object versions to the branch bucket, and you can modify settings such as bucket policies.

* *Read-only*: You can't modify objects in the branch bucket. You can modify bucket options and bucket access for the branch bucket. 
|===
...
+
On the Bucket details page, some configuration options are disabled for read-only buckets.

== Branch bucket operations and settings

You can create a branch bucket from an existing branch bucket.

A multipart uploaded object is included in a branch bucket if the ingest was initiated before the base bucket's before timestamp, regardless of when the upload completes.

If you delete object versions from a branch bucket, StorageGRID creates a delete marker only in the branch bucket. The base bucket object versions won't be affected.

Cross-grid replication and platform services aren't supported for branch buckets starting in StorageGRID 12.0.
