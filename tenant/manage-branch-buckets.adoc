...
---
= Manage StorageGRID branch buckets
:icons: font
:imagesdir: ../media/
...
Branch bucket depth limit:: A branch bucket can have a maximum of 20 base buckets. As branch-bucket depth grows, S3 access latency can increase, especially for ListObjectVersions calls.
+
* If you create a branch bucket from a base bucket that's a regular bucket (a bucket that was initially created as a base bucket), the new branch bucket has one base bucket.
* If the base bucket is itself a branch bucket, the new branch bucket inherits all of that branch bucket's base buckets. Users specify only one base bucket directly; the inherited base buckets are implied.
* The total number of base buckets (explicit and implied) can't exceed 20.
+
For example, assume bucket *A* is a regular bucket with versioning enabled:
+
** Branch bucket *B*, created from *A*, has one base bucket: *A*.
** Branch bucket *C*, created from *B*, has two base buckets: *B* (explicit) and *A* (implied).
** Branch bucket *D*, created from *C*, has three base buckets: *C* (explicit), *B* (implied) and *A* (implied).
**	This pattern continues up to a maximum depth of 20 base buckets.

S3 Object Lock permissions:: Permissions to set S3 Object Lock properties of buckets or objects can be granted by link:../tenant/using-s3-object-lock.html[bucket policy or group policy].

Suspended versioning:: If you suspend versioning on the base bucket, the contents of the base bucket will no longer be visible in its branch buckets.
...
Choose the time range to include object versions from the base bucket. For any time filter, select any date within the following range:

* Minimum date: January 1, 2010 (UTC)
* Maximum date: Five years from today's date (UTC)

Before:: The branch bucket provides access to object versions created earlier than _Before_ time.

After:: The branch bucket provides access to object versions created later than _After_ time.

Between:: The branch bucket provides access to object versions created between the _From_ and _To_ times.

Exclude:: The branch bucket doesn't provide access to object versions created between the _Exclude from_ and _Exclude to_ times.
