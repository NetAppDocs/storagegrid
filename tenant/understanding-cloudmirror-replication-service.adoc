...
---
= Learn about the StorageGRID CloudMirror replication service
:icons: font
:imagesdir: ../media/
...
NOTE: CloudMirror replication is supported only if the source bucket has S3 Object Lock enabled and the endpoint is an Amazon or StorageGRID S3 bucket. Object Lock settings are not replicated to the bucket endpoint.
