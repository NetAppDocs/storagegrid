...
---
= Configure CloudMirror replication in StorageGRID
:icons: font
:imagesdir: ../media/
...
* Replication is only supported for source or destination buckets with S3 Object Lock enabled for Amazon S3 or StorageGRID S3 buckets. The object lock headers are not replicated to the destination object.
...
* If you enable S3 Object Lock on the destination bucket after you configure CloudMirror replication, you might have to link:../tenant/creating-platform-services-endpoint.html[re-save the endpoint] to allow replication to resume.
