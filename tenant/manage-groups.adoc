...
+
* If any groups weren't synced to the tenant on the destination grid, follow the steps to <<sync-groups,retry a group sync>>.
...
.. In the Overview section, change the display name by selecting the name or the edit icon image:icon_edit_tm.png[Edit icon].
...
+
image:manage_users.png[Manage users]
.. Select *Add users*.
...
. On the banner, select *Retry sync*. This operation retries syncing for all groups that failed to sync.
. To retry syncing an individual group, select the group name. On the group details page, select *Retry sync*.
. View the status of the sync operation from the details page of each group you're syncing.
