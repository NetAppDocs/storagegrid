...
With AssumeRole you can allow a trusted user to access S3 resources and perform permitted S3 actions. Trusted users receive temporary security credentials that are valid for a limited duration. You provide users with a temporary access key and you can set their session to between 15 minutes and 12 hours. The default session is 1 hour. When you remove the user from the group's S3 trust policy, their AssumeRole permissions are terminated.
...
. Add your S3 trust policy.

. Select *Save changes*.

NOTE: The key difference between AssumeRole StorageGrid and AssumeRole AWS is that in StorageGrid we create groups, and in AWS, we create roles. 
It is useful to think of a StorageGrid group as a role group. 

// 2025-03-28, jira SGWS-34003