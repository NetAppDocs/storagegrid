...
+
* As needed, a banner message indicates if groups were not synced to the tenant on the destination grid. You can <<sync-groups,retry a group sync>> that failed.
...
.	Select each group that indicates _(Sync failed)_ below the group name.
. Select *Actions* > *Sync groups*
