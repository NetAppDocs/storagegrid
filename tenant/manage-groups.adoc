...
To set up AssumeRole, create the target group to be assumed, if the group does not already exist. Edit the group's S3 policy to specify the permitted actions by assuming this group. Edit the group's S3 trust policy to specify the trusted users allowed to assume the group with the AssumeRole API.
...
The final permissions are determined by group policy, bucket policy, and session policy. For more information see link:../s3/bucket-and-group-access-policies.html[Use bucket and group access policies].


// 2025-04-15, jira SGWS-34003