...
NOTE: s3Trust" is a new policy attribute added to the tenant group in 12.0. As part of Account clone, when cloning a tenant group from a 12.0 grid (source) to a 11.9 grid (destination), it'll fail with error, 'unrecognized attribute, "s3Trust"' on the 11.9 grid, hence, the clone fails. The failure is visible on the source grid. And the clone is retry able
