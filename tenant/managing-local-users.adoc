...
You can import one or more federated users, up to a maximum of 100 users.

.Steps

. Navigate to the *Access Management* > *Users* page. 
. Select *Import federated users*.
. Enter the UUID or shortname for one or more multiple federated users, and select *Import*.
+
The federated user information displays on the Users page.
. If the import into the Users field fails for one or more users:
.. Select *Copy Users*.
.. Retry the import by selecting *Previous*, and pasting the copied users to the *Import federated users* field.
