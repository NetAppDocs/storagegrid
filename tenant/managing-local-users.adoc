...
. If the import into the Users field fails for one or more users, the *Users not imported* panel displays:
.. Select *Copy Users*.
.. Retry the import by selecting *Previous*, and pasting the copied users into the *Import federated users* panel.
