...
For most endpoints, the final element of the URN identifies the specific target resource at the destination URI, for example, `sns-topic-name`. 

For webhook endpoints, the target resource is the destination URI itself.
...
*Note:* For webhook endpoints, the final element of the URN can be any string, as long as the URN of the endpoint is unique.
...
+
----
urn:kafka:optional:optional:kafka-topic-name
----
+
NOTE: Optionally, you can include `mysite` when specifying a Kafka endpoint: `urn:mysite:kafka:optional:optional:kafka-topic-name`
+
