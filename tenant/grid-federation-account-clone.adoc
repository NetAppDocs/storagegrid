...
image:grid_federation_account_clone_workflow_tm.png[the tenant's workflow for account clone. Steps are described in the following text.]
...
image:grid_federation_account_clone.png[image showing that local groups are cloned from source grid to destination grid]
...
image:grid_federation_local_user_clone.png[image showing that local users are cloned from source grid to destination grid]
...
image:grid_federation_federated_group_clone.png[image showing that federated groups are cloned from source grid to destination grid]
...
image:grid_federation_s3_access_key.png[image showing that s3 access keys can be optionally cloned from source grid to destination grid]
...
image:grid_federation_account_not_cloned.png[image showing that details on destination grid aren't cloned to source grid]
...
image:grid_federation_account_clone_edit_delete.png[image showing that edited or deleted details aren't cloned]
