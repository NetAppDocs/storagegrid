...
.. View the Capacity limit usage and Object count limit usage bars. If usage is 100% or near 100%, consider increasing the limit or deleting some objects.
...
* *Bucket options*: View or change option settings. Some settings can't be changed after the bucket is created.
...
** link:../tenant/creating-s3-bucket.html#object-count-limit[Object count limit]
...
** Configure link:configuring-cross-origin-resource-sharing-for-buckets-and-objects.html[CORS for buckets and objects] so the bucket and objects in the bucket will be accessible to web applications in other domains.
