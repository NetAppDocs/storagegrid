...
---
= Learn about platform services for StorageGRID
:icons: font
:imagesdir: ../media/
...
When using versioning, note that the ordering of object versions in the destination bucket is best effort and not guaranteed by the CloudMirror service, due to limitations in the S3 protocol. Additionally, destination buckets will maintain their own settings, and they might not provide versioning.
...
| If the source S3 bucket in StorageGRID has S3 Object Lock enabled, you should also enable S3 Object Lock for the destination bucket with default retention configured.

Replication of _object data_ is supported for source and destination buckets with S3 Object Lock enabled. However, destination buckets will maintain their own settings for object retention, and they might not provide compliance or versioning.
|===
