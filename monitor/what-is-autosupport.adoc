...
---
= Learn about AutoSupport in StorageGRID
:icons: font
:imagesdir: ../media/
...
Using AutoSupport helps solve problems faster. Technical support can monitor your system's storage needs and help determine if you need to add new nodes or sites. Optionally, AutoSupport can send packages to one additional destination.
...
+
NOTE: If StorageGRID AutoSupport isn't enabled, a message appears on the Grid Manager dashboard. The message includes a link to the AutoSupport configuration page. If you close the message, it won't appear again until your browser cache is cleared, even if AutoSupport remains disabled.
...
Active IQ Digital Advisor (also known as Digital Advisor) is cloud-based and leverages predictive analytics and community wisdom from NetApp's installed base. Its continuous risk assessments, predictive alerts, prescriptive guidance, and automated actions help you prevent problems before they occur, leading to improved system health and higher system availability.
