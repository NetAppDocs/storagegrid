...
---
= Monitor tenant activity in StorageGRID
:icons: font
:imagesdir: ../media/
...
+
image:tenant_accounts_page.png[Tenant Accounts page]

. Optionally, sign in to a tenant account by selecting the sign-in link image:icon_tenant_sign_in.png[Tenant sign-in icon] in the *Sign in/Copy URL* column.

. Optionally, copy the URL for a tenant's sign-in page by selecting the copy URL link image:icon_tenant_copy_url.png[Tenant copy URL icon] in the *Sign in/Copy URL* column.
...
+
image:tenant_accounts_example_csv.png[Tenant accounts example .csv]
+
...
+
image:tenant_usage_modal.png[Tenant usage modal]
...
+
image:tenant_bucket_space_consumption_GM.png[Tenant bucket consumption bar chart]
+
...
+
image:tenant_bucket_details_csv.png[Tenant bucket details CSV example]
+
You can open the `.csv` file in a spreadsheet application or use it in automation.
