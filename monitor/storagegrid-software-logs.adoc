...
NOTE: If you want to send your logs to an external syslog server or change the destination of audit information such as the `bycast.log` and `nms.log`, see link:../monitor/configure-log-management.html[Configure log management].
...
| The primary StorageGRID troubleshooting file.
| All nodes

m| /var/local/log/bycast-err.log
| Contains a subset of `bycast.log` (messages with severity ERROR and CRITICAL). CRITICAL messages are also displayed in the system.
| All nodes
...
* Contains logs related to AutoSupport.
| Admin Nodes
...
m| /var/local/log/cache-svc.log +
/var/local/log/cache-svc.errlog
| Cache service logs.
| Gateway Nodes
