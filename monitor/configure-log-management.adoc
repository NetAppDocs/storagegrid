...
---
= Configure log management in StorageGRID
:icons: font
:imagesdir: ../media/

[.lead]
As needed, configure audit levels, protocol headers, the location of audit messages and logs, and log retention.
...
Optionally, you can link:../monitor/considerations-for-external-syslog-server.html[configure an external syslog server] to save audit information remotely. You can minimize the performance impact of audit message logging without reducing the completeness of audit data by using an external server. An external syslog server is especially useful if you have a large grid, use multiple types of S3 applications, or want to retain all audit data.
...
+
When StorageGRID finds HTTP headers in a request, it includes them in the audit message under the field HTRH.
+
...
By default, audit messages and logs are saved on the nodes where they're generated. StorageGRID regularly rotates and eventually deletes audit messages and logs to prevent them from consuming excessive disk space. To save audit messages and a subset of logs externally, <<use-external-syslog-server,use an external syslog server>>.
...
NOTE: If you don't want to use an external syslog server, skip this step and go to <<select-log-location,Select log location>>.
...
+
If you select a value, StorageGRID applies the selected value to all messages of this type. You lose details about different severities if you override severity with a fixed value.
+
[cols="1a,3a" options="header"]
|===
| Severity
...
Before using an external syslog server, request that all nodes in your grid send test messages to the external syslog server. These test messages help you validate your entire log collection infrastructure before you commit to sending data to the external syslog server.
...
You can manage how long StorageGRID retains logs by creating an ILM rule or configuring an S3 bucket lifecycle policy.

[role="tabbed-block"]
====
.ILM rule
--
Create an ILM rule for the bucket and add this rule to the active ILM policy.

.Steps

. Select *ILM* > *Rules*.
. Select *Create*. link:../ilm/create-ilm-rule-enter-details.html[Step 1 (Enter details)] of the Create an ILM rule wizard appears:
.. Enter a unique name for the rule. For example: ILM Rule for archived logs.
.. Select the log archival S3 tenant account.
.. Specify the log archival S3 bucket.
. Continue to link:../ilm/create-ilm-rule-define-placements.html[Step 2 (Define placements)] and define the placement for the archived logs rule. For example, the ILM rule stores one replicated copy in Site 1 and one replicated copy in Site 2 for the first year. After one year, a 2+1 erasure-coded copy is made and saved at only one site.
. After creating the rule, add it to an active ILM policy.
--

.S3 bucket lifecycle
--
Configure an S3 bucket lifecycle policy on the log archival S3 bucket:
...
When you configure an S3 bucket lifecycle policy on the log archival S3 bucket, the lifecycle expiration actions override the ILM policy for objects matching the bucket lifecycle filter. Objects that don't match the bucket lifecycle filter use the retention settings of the ILM policy. If an object matches a bucket lifecycle filter and no expiration actions are explicitly specified, the retention settings of the ILM policy aren't used and it's implied that object versions are retained forever.
--
====

// 2025 Dec 18, SGRIDDOC-215
