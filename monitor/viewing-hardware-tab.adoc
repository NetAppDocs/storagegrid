...
---
= View the StorageGRID Hardware tab
:icons: font
:imagesdir: ../media/
...
image:nodes_page_hardware_tab_graphs.png[Nodes page Hardware tab]
...
image:nodes_page_memory_usage_details.png[Nodes Page > Hardware >Memory Usage Details]
...
+
image:nodes_page_overview_tab_extended.png[Nodes Page Overview Extended]
+
...
|The status of power supply A for the compute controller.

|Compute controller power supply B
|The status of power supply B for the compute controller.
|===
+
[cols="1a,2a" options="header"]
|===
...
+
image:nodes_page_network_traffic_graph.png[Nodes Page Network Traffic Graph]
...
+
image:nodes_page_storage_used_object_data.png[Storage Used - Object Data]
+
image:storage_used_object_metadata.png[Storage Used - Object Metadata]
...
+
image:nodes_page_overview_tab_services_appliance.png[Nodes page Overview tab for services appliance]

+
The Alerts section of the Overview tab displays any active alerts for the node.

. Select the *Hardware* tab to see more information about the appliance.
 .. View the CPU Utilization and Memory graphs to determine the percentages of CPU and memory usage over time. To display a different time interval, select one of the controls above the chart or graph. You can display the information available for intervals of 1 hour, 1 day, 1 week, or 1 month. You can also set a custom interval, which allows you to specify date and time ranges.
+
image:nodes_page_hardware_tab_graphs_services_appliance.png[Nodes page Hardware tab graphs for services appliance]

 .. Scroll down to view the table of components for the appliance. The table contains information such as the model name, serial number, controller firmware version, and the status of each component.
+
[cols="1a,2a" options="header"]
|===
| Field in the Appliance table| Description
...
. Select the *Network* tab to view information for each network.
+
The Network Traffic graph provides a summary of overall network traffic.
+
image:nodes_page_network_traffic_graph.png[Nodes Page Network Traffic Graph]
...
. Select the *Storage* tab to view information about the disk devices and volumes on the services appliance.
