...
| A problem was detected with a CMOS battery in the appliance.
...
| Appliance DAS drive is in the wrong slot or node
| A direct-attached storage (DAS) drive is in the wrong slot or node
...
| Appliance SANtricity OS software update required
| SANtricity software version is lower than the recommended minimum for this release of StorageGRID.

| Appliance SFP fault detected
| An SFP fault has been detected in the compute controller of a StorageGRID appliance.
...
| ListBucket permission check for Get/Head object disabled
| ListBucket permission check for GetObject/HeadObject on non-existent keys is disabled.
|===
...
| Log archival destination bucket not found
| The log archival destination bucket is missing, which prevents logs from being archived to the destination bucket.

| SNMP inform errors
| Errors sending SNMP inform notifications to a trap destination.

| SSH external access enabled
| SSH external access has been enabled for more than 24 hours.
...
| Chunk service high goroutine count
| The chunk service has an unusually high number of goroutines, which might indicate a deadlock or resource leak.
...
| Erasure-coded groups in Needs Repair state
| One or more erasure-coded groups are in the Needs Repair state when no erasure-coded data repair is in progress.
|===
...
| ADC quorum not met
| Storage Node with ADC service is offline. Expansion and decommission operations are blocked until ADC quorum is restored.
...
| Objects potentially lost
| One or more objects potentially lost from the grid.

| Orphan objects detected
| Orphan objects have been detected.

| S3 multipart upload part size too large
| A client is attempting a multipart upload operation that exceeds the part size limit.

| S3 PUT object size too large
| A client is attempting a PUT Object operation that exceeds S3 size limits.

| S3 requests using unsupported conditional headers
| An S3 client is using unsupported conditional headers (If-Match or If-None-Match) for certain operations.

| S3 requests using wrong region with global namespace enabled
| An S3 client is using the wrong signing region to access this grid's buckets.
...
| Unexpected object size detected during object existence check procedure.
|===
...
| The security policy doesn't require FIPS mode, but FIPS modules are in use.

| Reboot required to enable FIPS
| The security policy requires FIPS mode, but FIPS modules aren't in use.
