...
summary: 'The AutoSupport feature enables your StorageGRID system to send periodic and event-driven health and status messages to technical support to allow proactive monitoring and troubleshooting. StorageGRID AutoSupport also enables the use of Digital Advisor for predictive recommendations.'
---
= Configure AutoSupport in StorageGRID
:icons: font
:imagesdir: ../media/

[.lead]
The AutoSupport feature enables your StorageGRID system to send periodic and event-driven health and status messages to technical support to allow proactive monitoring and troubleshooting. StorageGRID AutoSupport also enables the use of Digital Advisor for predictive recommendations.

By default, the weekly and event-triggered AutoSupport features are enabled for software-based StorageGRID systems when you first install StorageGRID. However, you must <<autosupport-for-appliances,configure hardware AutoSupport>> on each appliance. As needed, you can change the AutoSupport configuration.

NOTE: If you want to change the configuration of StorageGRID AutoSupport, make your changes only on the primary Admin Node.
...
* If you will use HTTPS for sending AutoSupport packages, you have provided outbound internet access to the primary Admin Node, either directly or link:../admin/configuring-admin-proxy-settings.html[using a proxy server] (inbound connections not required).
* If HTTP is selected on the StorageGRID AutoSupport page, you have link:../admin/configuring-admin-proxy-settings.html[configured a proxy server] to forward AutoSupport packages as HTTPS. NetApp's AutoSupport servers will reject packages sent using HTTP.
//11mar2025: SG still allows HTTP. In Aug 2023 (SG 11.9), setting up a proxy was introduced.
* If you will use SMTP as the protocol for AutoSupport packages, you have link:../monitor/configure-email-server.html[configured an email server].
...
* *Log collection*: link:../monitor/collecting-log-files-and-system-data.html[manually collect log files and system data and send an AutoSupport package].

== [[configure-autosupport]]Configure AutoSupport

To change the installation default configuration for AutoSupport, or to configure hardware AutoSupport on each StorageGRID appliance:
...
* *HTTP*: If you select HTTP, you must link:../admin/configuring-admin-proxy-settings.html[configure a proxy server] to forward AutoSupport packages as HTTPS. NetApp's AutoSupport servers reject packages sent using HTTP. This protocol uses port 80. 
* *SMTP*: Use this option if you want AutoSupport packages to be emailed. You must first link:../monitor/configure-email-server.html[configure an email server].

+
The protocol you set is used for sending all AutoSupport packages except AutoSupport on Demand, which requires the HTTPS protocol.
...
. If you selected *SMTP*, use the *Sender* field to enter the address for the email server you configured earlier.
...
To verify or change the protocol used to send AutoSupport packages, see the instructions to <<configure-autosupport,configure AutoSupport>>.
...
AutoSupport packages from E-Series appliances can be included in StorageGRID AutoSupport if you configure AutoSupport delivery by proxy in link:sending-eseries-autosupport-messages-through-storagegrid.html[SANtricity System Manager].
...
* link:../network/external-communications.html[Ports used for external communications]
* https://mysupport.netapp.com/site/global/dashboard[NetApp Support^]

// 2023 NOV 21, SGRIDDOC-29
// 2026-03-30, SGRIDDOC-147