...
NOTE: If you configured a proxy server to forward AutoSupport packages to NetApp, link:../admin/configuring-admin-proxy-settings.html[verify that the proxy server configuration settings are correct].
...
If a failure occurs and SMTP is the selected protocol, the following error message might appear on the AutoSupport page: `AutoSupport packages cannot be sent using SMTP protocol due to incorrect settings on the E-mail Server page.`
