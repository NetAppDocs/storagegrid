...
BLDR
a|
HSTE and HSTU are related to HTTP for all LDR traffic, including S3 and other internal StorageGRID traffic. An alarm indicates that one of the following situations has occurred:
...
. link:../admin/configuring-custom-server-certificate-for-storage-node.html#add-a-custom-s3-api-certificate[Upload a new S3 API certificate.]
