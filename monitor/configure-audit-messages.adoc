...
---
= Configure audit message settings
:icons: font
:imagesdir: ../media/
...
By default, audit messages and logs are saved on the nodes where they're generated. If you want to save audit messages and a subset of logs externally, <<use-external-syslog-server,use an external syslog server>>.

Logs are periodically rotated. To store logs for an extended period of time, enable archive logs, and then choose a tenant and bucket for log storage.
...
Logs are periodically rotated. Use an S3 bucket to store logs for an extended period of time.
...
. Select *Save*.
+
Logs will be stored in the tenant and bucket you specified.
