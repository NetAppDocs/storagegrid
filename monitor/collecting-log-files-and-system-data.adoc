...
+
If you select a very long time period or collect logs from all nodes in a large grid, the log archive could become too large to be stored on a node, or too large to be collected to an Admin Node for download. If either scenario occurs, restart log collection with a smaller set of data.
