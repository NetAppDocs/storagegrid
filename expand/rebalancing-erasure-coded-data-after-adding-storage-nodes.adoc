...
'
---
= Rebalance erasure-coded data after adding nodes to your StorageGRID system
:icons: font
:imagesdir: ../media/
...
** The data in all nodes is within a 5% deviation of the usable space partition or available space partition.     
...
+
Go to the Nodes page to assess the data distribution across the Storage Nodes in the site.
+ 
The EC rebalance job is considered complete and will stop when the usable space in all nodes is within a 5% deviation of the target usable space partition.   

** If `State` is `Success`, optionally <<review_object_storage,review object storage>> to see the updated details for the site.
+
Usable space should now be more balanced among the Storage Nodes at the site.
