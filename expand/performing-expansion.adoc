...
---
= Perform a StorageGRID expansion 
:icons: font
:imagesdir: ../media/
...
+
image:grid_expansion_page.png[screenshot showing Grid Expansion page]
...
+
image:grid_node_details.png[screenshot of Grid Node details]
+
...
| Whether this Storage Node will run the Administrative Domain Controller (ADC) service. The ADC service keeps track of the location and availability of grid services. At least three Storage Nodes at each site must include the ADC service.

* Select *Yes* if the Storage Node you are replacing includes the ADC service. Because you can't decommission a Storage Node if too few ADC services would remain, this setting ensures that a new ADC service is available before the old service is removed.
* You can link:../maintain/move-adc-service.html[move the ADC service to another Storage Node on the same site] to ensure the ADC service quorum is met.
