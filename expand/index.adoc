...
Before decommissioning or expanding a grid node, you must confirm that no data repair jobs are active. If any repairs have failed, you must restart them and allow them to complete before performing the decommission or expansion procedure. See link:../maintain/checking-data-repair-jobs.html[Check data repair jobs] for guidance on decommissioning a grid node.
