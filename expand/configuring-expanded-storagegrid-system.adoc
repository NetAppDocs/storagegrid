...
.. If single sign-on (SSO) is enabled for your StorageGRID system, create a relying party trust for the new Admin Node. You can't sign in to the node until you create this relying party trust. Refer to configure SSO using link:../admin/configure-sso-saml.html[SAML] or link:../admin/configure-sso-oidc.html[OIDC].
