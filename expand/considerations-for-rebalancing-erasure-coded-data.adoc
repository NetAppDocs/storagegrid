...
---
= Learn about EC rebalancing after expansion in StorageGRID
:icons: font
:imagesdir: ../media/
...
* Attempts to distribute bytes of data so each node has the same usable space. Nodes that contain more replicated data might store less erasure-coded data after the rebalance is complete.
* Redistributes usable space evenly between Storage Nodes without considering the relative capacities of each node. Replicated data is included in the calculation.
* The rebalance procedure is considered complete if it can't move any more erasure-coded data, even if the usable space is still not evenly distributed.
* Won't distribute erasure-coded data to Storage Nodes that are more than 80% full or to metadata-only Storage Nodes.
...
** Larger-capacity nodes might be relatively less full than smaller-capacity nodes, even though all nodes will end up with approximately the same amount of usable space.
+
For example, with a 2+1 Erasure Coding scheme, suppose three 200-TB nodes are each filled to 80% (200 × 0.8 = 160 TB on each node, or 480 TB for the site). If you add a 400-TB node and run the rebalance procedure, all the older nodes will now have approximately the same amount of erasure-coded data (106.66 TB or 53% used). The new node will have (160 TB of data or 40% used). In this case, the Used (%) for the larger node will be less than the Used (%) for the smaller nodes. The rebalance procedure completes before the usable space is balanced as no more erasure-coded data can be moved without breaking the erasure-coding scheme constraints.
+
image:used_space_with_larger_node.png[Used space before expansion]

== When to rebalance erasure-coded data

The EC rebalance procedure redistributes existing erasure-coded data to ensure nodes don't become or stay full. The procedure helps to ensure that EC encoding can continue on the site.

Run the rebalance procedure when there's a concerning bias in the data distribution on a site and the site stores mostly EC data (since replicated data cannot be moved by rebalance).
...
+
image:used_space_before_expansion.png[Used space before expansion]

=== Rebalance isn't required if you add enough nodes
...
image:used_space_after_3_node_expansion.png[Used space after 3-node expansion]
...
image:used_space_after_2_node_expansion.png[Used space after 2-node expansion]

To make use of the new Storage Nodes, you should run the EC rebalance procedure. When this procedure runs, StorageGRID redistributes existing erasure-coded data and parity fragments so usable space is evenly distributed among all Storage Nodes at the site. In this example, when the EC rebalance procedure is complete, all five nodes are now 60% full with 40% usable space, and objects can continue to be ingested into the 2+1 erasure-coding scheme on all Storage Nodes.

image:used_space_after_ec_rebalance.png[Used space aAfter EC rebalance]
