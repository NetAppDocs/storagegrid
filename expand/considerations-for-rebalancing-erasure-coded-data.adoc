...
* Attempts to distribute the same number of bytes to each node. Nodes that contain more replicated data will  store less erasure-coded data after rebalance is complete.
* Redistributes erasure-coded data evenly between Storage Nodes without considering the relative capacities of each node. Replicated data is included in the calculation.
