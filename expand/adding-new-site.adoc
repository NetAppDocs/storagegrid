...
* All sites must include at least three Storage Nodes. They can be any combination of Storage Nodes (data-only, metadata-only, or combined), as long as three ADC services are available. For more information, refer to link:../primer/what-storage-node-is.html#required-storage-nodes-per-grid-and-per-site[Required Storage Nodes per grid and per site].
