...
+
*** *UserPrincipalName pattern (Active Directory and Entra ID)*: `[USERNAME]@_example_.com`
*** *Down-level logon name pattern (Active Directory and Entra ID)*: `_example_\[USERNAME]`
 ...
 ** *Do not use TLS*: The network traffic between the StorageGRID system and the LDAP server will not be secured. This option isn't supported for Microsoft Entra ID.
+
