...
** CompleteMultipartUpload: `x-amz-checksum-crc32`, `x-amz-checksum-crc32c`, and `x-amz-checksum-crc64nvme`
** CreateMultipartUpload: `CRC32`, `CRC32C`, and `CRC64NVME` for `x-amz-checksum-algorithm`
** CopyObject: `CRC32`, `CRC32C`, `CRC64NVME`, and `SHA256` for `x-amz-checksum-algorithm`
...
* Added support for the `s3:x-amz-copy-source` policy condition key.
* Allow S3 Object Lock to be enabled on existing buckets.

|12.0
|
* Added support for cross-origin resource sharing (CORS) for a management interface, which allows another domain to access data in StorageGRID using management APIs. link:../admin/enable-cross-origin-resource-sharing-for-management-interface.html[Learn more].

* Added support for STS AssumeRole and session policy. See link:../s3/example-session-policies.html[an example of a session policy]. You can set up AssumeRole under tenant groups. 
...
|11.7
|
* Added link:../s3/quick-reference-support-for-aws-apis.html[Quick reference: Supported S3 API requests].
