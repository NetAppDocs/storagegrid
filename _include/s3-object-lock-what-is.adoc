...
When the global S3 Object Lock setting is enabled for a StorageGRID system, an S3 tenant account can create buckets with or without S3 Object Lock enabled. When the S3 Object Lock setting for existing buckets is enabled, an S3 tenant account can enable S3 Object Lock for an existing bucket. If a bucket has S3 Object Lock enabled, bucket versioning is required and is enabled automatically. After an S3 tenant account enables S3 Object Lock for a new or existing bucket, it can't be disabled.
