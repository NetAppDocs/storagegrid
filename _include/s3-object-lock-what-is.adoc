...
When the global S3 Object Lock setting is enabled for a StorageGRID system, an S3 tenant account can create buckets with or without S3 Object Lock enabled. When the allow S3 Object Lock for existing buckets setting is enabled, an S3 tenant account can enable S3 Object Lock for an existing bucket. If a bucket has S3 Object Lock enabled, bucket versioning is required and is enabled automatically. After an S3 tenant account enables S3 Object Lock for a new or existing bucket, it can't be disabled.
...
+
For details on the object settings, refer to link:../s3/use-s3-api-for-s3-object-lock.html[Use S3 REST API to configure S3 Object Lock].
...
Refer to link:../tenant/creating-s3-bucket.html[Create an S3 bucket] and link:../tenant/update-default-retention-settings.html[Update S3 Object Lock default retention].
