...
=== Installation password (temporary) key
...
| For the primary Admin Node, set a default temporary password for the StorageGRID Management API during installation.

Setting this value has no effect when:

* You set it on nodes other than the primary Admin Node.
* Installation has completed.

If this key is omitted, no temporary password will be set by default. Alternatively, you can set a temporary password using the StorageGRID Installation API.
