//shared procedure for restoring object data

.Before you begin

* If the automatic procedure failed to remount preserved storage volumes and reformat failed storage volumes, you've performed one of these procedures:
...
* You've confirmed that the Storage Node where you are restoring objects has a Connection State of *Connected* image:../media/icon_alert_green_checkmark.png[icon alert green check mark] on the *Nodes* > *Overview* tab in the Grid Manager.

* You've confirmed the following:
** A grid expansion to add a Storage Node isn't in process.
** A Storage Node decommission isn't in process or failed.
** A recovery of a failed storage volume isn't in process.
** A recovery of a Storage Node with a failed system drive isn't in process.
** An EC rebalance job isn't in process.
** Appliance node cloning isn't in process.

.About this task

After you've replaced the drives and the volumes have been formatted, Grid Manager displays the volumes as candidates for restoration on the *Maintenance* > *Volume restoration* > *Nodes to restore* tab.
...
+
If the recovered Storage Node contains fewer volumes than the node it's replacing, contact technical support for assistance.
...
** If an ILM rule was configured to store only one replicated copy and that copy existed on a storage volume that failed, you won't be able to recover the object.
...
[NOTE]
====
* You won't be able to start volume restoration jobs manually on any nodes.
...
+
Each volume in the node must indicate that it's ready to restore.
...
Data repair rates for replicated and erasure-coded objects in all volumes are averages summarizing all restorations in process. The percentage of objects in those volumes that are intact and don't require restoration is also indicated.
