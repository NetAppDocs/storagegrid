...
NOTE: Support for port remapping is deprecated and will be removed in a future release. To remove remapped ports, see link:../maintain/removing-port-remaps.html[removing port remaps on StorageGRID appliances] or link:../maintain/removing-port-remaps-on-bare-metal-hosts.html[removing port remaps on bare metal hosts].
