...
---
= External communications for StorageGRID
:icons: font
:imagesdir: ../media/
...
** link:../install-nodes/creating-node-configuration-files.html[Port remap keys for StorageGRID on Red Hat Enterprise Linux]

** link:../install-nodes/deploying-storagegrid-node-as-virtual-machine.html#vmware-remap-ports[Remap ports for StorageGRID on VMware]
...
*Note*: This port is only required when you need to enable SSH access for certain maintenance operations.
| 25
| TCP
| SMTP
| Admin Nodes
...
*Note*: If you close Grid Manager ports 443 or 8443, any users currently connected on a blocked port, including you, lose access to Grid Manager unless their IP address has been added to the Privileged address list. Refer to link:../admin/configure-firewall-controls.html[Configure firewall controls] to configure privileged IP addresses.
