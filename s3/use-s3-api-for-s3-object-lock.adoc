...
---
= Use S3 REST API to configure StorageGRID S3 Object Lock
:icons: font
:imagesdir: ../media/
...
NOTE: StorageGRID doesn't support the `x-amz-sdk-checksum-algorithm` request header.
...
To enable S3 Object Lock for a bucket, use one of these methods:
...
* Manage bucket settings from the Tenant Manager. See link:../tenant/update-default-retention-settings.html[Update S3 Object Lock default retention].
...
When new object versions are ingested to the bucket, the default retention mode is applied if `x-amz-object-lock-mode` and `x-amz-object-lock-retain-until-date` aren't specified. The default retention period is used to calculate the retain-until-date if `x-amz-object-lock-retain-until-date` isn't specified.

If the default retention period is modified after ingest of an object version, the retain-until-date of the object version remains the same and isn't recalculated using the new default retention period.

You must have the `s3:PutBucketObjectLockConfiguration` permission, or be account root, to complete this operation.

You must specify a `Content-MD5`  or `x-amz-checksum-*` request header in the PutObjectLockConfiguration request. However, StorageGRID doesn't validate the contents of the `x-amz-checksum-*` header for this operation.

==== Request example
This example enables S3 Object Lock for a bucket (if your grid administrator has given permission) and sets the default retention mode to COMPLIANCE and the default retention period to 6 years.
...
When new object versions are ingested to the bucket, the default retention mode is applied if `x-amz-object-lock-mode` isn't specified. The default retention period is used to calculate the retain-until-date if `x-amz-object-lock-retain-until-date` isn't specified.
...
You must specify a `Content-MD5`  or `x-amz-checksum-*` request header in the request. However, StorageGRID doesn't validate the contents of the `x-amz-checksum-*` header for these operations.
...
* You must include a `Content-MD5` or `x-amz-checksum-*` request header if any `x-amz-object-lock-*` request header is present in the PutObject request. These headers aren't required for CopyObject or CreateMultipartUpload.
* If the bucket doesn't have S3 Object Lock enabled and a `x-amz-object-lock-*` request header is present, a 400 Bad Request (InvalidRequest) error is returned.
...
+
The `x-amz-bypass-governance-retention` header has no effect on objects without S3 Object Lock.
