...
You must specify one of the following request headers in the PutObjectLockConfiguration request:
...
NOTE: StorageGRID doesn't validate the contents of the x-amz-checksum-* header for this operation.
...
* You must include one of the following request headers if any x-amz-object-lock-* request header is present in the PutObject request:
+
----
Content-MD5
x-amz-checksum-*
----
+
These headers aren't required for CopyObject or CreateMultipartUpload.
