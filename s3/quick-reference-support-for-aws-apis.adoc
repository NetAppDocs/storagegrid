...
---
= Supported S3 API requests in StorageGRID
:icons: font
:imagesdir: ../media/
...
This page includes only the S3 operations that are supported by StorageGRID. Any S3 operations not listed might return `501 Not Implemented` or result in unintended behavior.

TIP: To see the AWS documentation for each operation, select the link in the heading.
...
link:use-access-policies.html[Use bucket and group access policies].
...
.URI query parameters and request headers
StorageGRID supports _most_ <<common-params, common parameters and headers>> for this request.

.Request body
StorageGRID supports _most_ request body parameters defined by the Amazon S3 REST API at the time of implementation, with the following exception:

You can't use PutObjectLockConfiguration to enable S3 Object Lock on an existing bucket unless your grid administrator has given permission.
