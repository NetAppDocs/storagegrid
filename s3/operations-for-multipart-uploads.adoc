...
** *Before*. If the ingest was initiated before the base bucket's _Before_ timestamp, the multipart uploaded object is included, regardless of when the upload completes.
** *After*. If the ingest was initiated after the base bucket's _After_ timestamp, the multipart uploaded object is included.
** *Between*. If the ingest was initiated between the base bucket's _From_ and _To_ timestamps, the multipart uploaded object is included, regardless of when the upload completes.
** *Exclude*. If the ingest was initiated before the _Exclude from_ time or after the _Exclude to_ time, the multipart uploaded object is included.
