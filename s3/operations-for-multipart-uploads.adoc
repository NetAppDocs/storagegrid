...
* A multipart uploaded object is included in a link:../tenant/manage-branch-buckets.html[branch bucket] if the ingest was initiated before the base bucket's before timestamp, regardless of when the upload completes.
