...
---
= How StorageGRID implements S3 REST API operations for objects
:icons: font
:imagesdir: ../media/
...
* The `If-None` and `If-None-Match` headers are accepted but non-functional. Depending on your StorageGRID version and configuration, providing these headers might return an error message (501 Not Implemented). If the `If-None` or `If-None-Match` header is accepted but non-functional, an alert might trigger with instructions about enabling the stricter 501 Not Implemented error behavior.
