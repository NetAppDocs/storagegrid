...
* The `If-None` and `If-None-Match` headers behave differently depending on your StorageGRID configuration. Providing these headers might return an error message (501 Not Implemented). If the `If-None` or `If-None-Match` headers are accepted but non-functional, an alert might trigger with instructions about enabling the stricter 501 Not Implemented error behavior.
