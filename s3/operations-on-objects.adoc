...
* The `If-None` and `If-None-Match` headers are accepted but non-functional. Depending on your StorageGRID version and  configuration, an error message (501 Not Implemented) might display or an alert might trigger with instructions about how to enable the behavior.
