...
By default, StorageGRID guarantees read-after-write consistency for newly created objects. Any GET following a successfully completed PUT will be able to read the newly written data. Overwrites of existing objects, metadata updates, and deletes are eventually consistent.

If you want to perform object operations at a different consistency, you can:

* Specify a consistency for <<bucket-consistency,each bucket>>.
...
** In the Grid Manager, go to *CONFIGURATION* > *System* > *Storage settings* > *Default bucket consistency*.
...
The consistency affects how the metadata that StorageGRID uses to track objects is distributed between nodes. Consistency affects the availability of objects for client requests.
...
* *Strong-global*: Guarantees read-after-write consistency for all client requests across all sites, with the following exceptions when Quorum semantics are configured:
** Two-site grids won't have site failure tolerance. Those grids will incur cross-site latency for all requests.
** For two-site grids, account usage won't be updated when a site is down.
** This feature doesn't change the behavior of GET Bucket consistency and PUT Bucket consistency operations. These operations won't work with one site down.
...
== [[bucket-consistency]]Specify consistency for bucket
...
== [[how-consistency-and-ILM-rules-interact]]How consistency and ILM rules interact to affect data protection
