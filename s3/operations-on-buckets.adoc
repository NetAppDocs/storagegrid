...
summary: 'The StorageGRID system supports a maximum of 20,000 buckets for each S3 tenant account.'
---
= S3 bucket operations and implementation details in StorageGRID
:icons: font
:imagesdir: ../media/

[.lead]
The StorageGRID system supports a maximum of 20,000 buckets for each S3 tenant account.

Each grid can have a maximum of 100,000 buckets.

If multiple tenants each have up to 5,000 buckets, each Storage Node in the grid must have a minimum of 64 GB of RAM.
...
You can check whether updates to last access time are enabled or disabled for individual buckets. Refer to link:get-bucket-last-access-time-request.html[GET Bucket last access time].
...
*Note*: S3 directory buckets are not supported.
...
+
*Note*: An error will occur if your CreateBucket request uses a region that hasn't been defined in StorageGRID.

* You can include the `x-amz-bucket-object-lock-enabled` request header to create a bucket with S3 Object Lock enabled. Refer to link:../s3/use-s3-api-for-s3-object-lock.html[Use S3 REST API to configure S3 Object Lock].
+
S3 Object Lock requires bucket versioning, which is enabled automatically when you create the bucket with S3 Object Lock enabled.
+
You can enable S3 Object Lock when you create the bucket or at a later time.
...
| Deletes the lifecycle configuration from the bucket. Refer to link:create-s3-lifecycle-configuration.html[Create S3 lifecycle configuration].
...
| Returns the lifecycle configuration for the bucket. Refer to link:create-s3-lifecycle-configuration.html[Create S3 lifecycle configuration].

| GetBucketLocation
| Returns the region that was set using the `LocationConstraint` element in the CreateBucket request. If the bucket's region is `us-east-1` (default), GetBucketLocation returns an empty value for `LocationConstraint`.
...
*Caution*: If a non-default ILM policy tag is set for this bucket, there will be a `NTAP-SG-ILM-BUCKET-TAG` bucket tag with a value assigned to it. Don't modify or remove this tag.
...
Refer to link:../s3/use-s3-api-for-s3-object-lock.html[Use S3 REST API to configure S3 Object Lock].
...
Refer to link:create-s3-lifecycle-configuration.html[Create S3 lifecycle configuration]. To understand how the Expiration action in a bucket lifecycle interacts with ILM placement instructions, refer to link:../ilm/how-ilm-operates-throughout-objects-life.html[How ILM operates throughout an object’s life].
...
| PutBucketPolicy
| Sets the policy attached to the bucket. Refer to link:use-access-policies.html[Use bucket and group access policies].
...
* Destination buckets must be specified as the URN of StorageGRID endpoints as specified in the Tenant Manager or the Tenant Management API. Refer to link:../tenant/configuring-cloudmirror-replication.html[Configure CloudMirror replication].
+
...
*Caution*: If a non-default ILM policy tag is set for this bucket, there will be a `NTAP-SG-ILM-BUCKET-TAG` bucket tag with a value assigned to it. Make sure that the `NTAP-SG-ILM-BUCKET-TAG` bucket tag is included with the assigned value in all PutBucketTagging requests. Don't modify or remove this tag.
...
* If the grid administrator has given permission, enables S3 Object Lock on existing buckets.

If the default retention period is modified, the retain-until-date of existing object versions remains the same and isn't recalculated using the new default retention period.

Refer to link:../s3/use-s3-api-for-s3-object-lock.html[Use S3 REST API to configure S3 Object Lock] for detailed information.
|===

// 2026 MAY 18, SGRIDDOC-252
// 2026 APR 21, SGWS-40729
// 2026 MAR 12, SGRIDDOC-223
