...
** `x-amz-server-side-encryption`
+
When you configure `x-amz-server-side-encryption` grid-wide, StorageGIRD doesn't return the `serversideencryption` header in the response but the encryption is still valid.
