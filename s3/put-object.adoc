...
** `x-amz-server-side-encryption`
+
When the `x-amz-server-side-encryption` header isn't included in the PutObject request, the grid-wide link:../admin/changing-network-options-object-encryption.html[stored object encryption] is omitted from the PutObject response.
