...
* `Content-Encoding`
+
When you specify `aws-chunked` for `Content-Encoding`, StorageGRID doesn't verify the following items:

** StorageGRID doesn't verify the `chunk-signature` against the chunk data.
** StorageGRID doesn't verify the value that you provide for `x-amz-decoded-content-length` against the object.
