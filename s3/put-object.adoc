...
** `x-amz-server-side-encryption`
+
When the `x-amz-server-side-encryption` header is not included in the PutObject request, the grid-wide link:../admin/changing-network-options-object-encryption.html[stored object encryption] is omitted from the PutObject response.
...
https://docs.aws.amazon.com/AmazonS3/latest/API/API_PutObject.html[Amazon Simple Storage Service API Reference for PutObject^]
