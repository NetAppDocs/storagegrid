...
* `x-amz-checksum-mode`
+ 
The `Range` header is not supported with `x-amz-checksum-mode` for GetObject. When you include `Range` in the request with `x-amz-checksum-mode` enabled, a checksum value is not returned in the response.
