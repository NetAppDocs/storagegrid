...
---
= Retrieve an object from a bucket in StorageGRID with the GetObject request
:icons: font
:imagesdir: ../media/
...
* `Accept-Encoding`
+
When an object has no stored `Content-Encoding`, a GET request with `Accept-Encoding: gzip` applies on the fly compression to the response. Only `gzip` affects the GET response encoding; `deflate`, `br`, and `identity` don't. For objects that already have a stored `Content-Encoding`, StorageGRID returns the stored encoding and doesn't apply additional on the fly compression based on `Accept-Encoding`. StorageGRID doesn't support range read operations for GET requests with `Accept-Encoding: gzip`.
