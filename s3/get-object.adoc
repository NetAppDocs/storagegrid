...
* `Accept-Encoding`
+
When an object doesn't store a `Content-Encoding` value, a GET request with `Accept-Encoding: gzip` automatically performs compression. Only the `gzip` value affects the object's content encoding. StorageGRID doesn't support Range read operations for GET requests with `Accept-Encoding: gzip`.
