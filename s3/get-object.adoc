...
+ 
The `partNumber` parameter and `Range` header are not supported with `x-amz-checksum-mode`. When you include them in the request, they do not return a checksum value for SHA256 in the response.
