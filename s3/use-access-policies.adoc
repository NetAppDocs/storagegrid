...
summary: "StorageGRID uses the Amazon Web Services (AWS) policy language to allow S3 tenants to control access to buckets and objects within those buckets."
---
= How StorageGRID uses AWS policies to control access to S3 buckets and objects
:icons: font
:imagesdir: ../media/
...
include::../_include/principal-in-s3-bucket-policy.adoc[]
...
A session policy is an access policy that temporarily restricts the permissions available during a specific session, such as when a user assumes a group. A session policy can only allow a subset of permissions, and it can't grant additional permissions. The group itself might have broader permissions.
...
* International characters, which can be specified in the object key, should be encoded using JSON UTF-8 or using JSON \u escape sequences. Percent-encoding isn't supported.
+
...
* The resource value can specify a bucket that doesn't yet exist when a group policy is created.
...
* The principal value can specify a group/user name that doesn't yet exist when a bucket policy is created.
...
== [[use-putoverwriteobject-permission]]Use PutOverwriteObject permission
 ...
 ** If an existing object isn't found, this permission has no effect.
* When this permission isn't present, the effect is the same as if Allow were set.
...
| IfExists
| Appended to any condition operator, except the Null condition, to check for absence of that condition key. Returns TRUE if the condition key isn't present.
|===
...
| aws:SourceIp
| IP operators
| Compares to the IP address from which the request was sent. Can be used for bucket or object operations.

*Note:* If the S3 request was sent through the Load Balancer service on Admin Nodes and Gateways Nodes, this compares to the IP address upstream of the Load Balancer service.

*Note*: If a third-party, non-transparent load balancer is used, this compares to the IP address of that load balancer. Any `X-Forwarded-For` header is ignored because its validity can't be ascertained.

| aws:username
| Resource/Identity
| Compares to the sender's username from which the request was sent. Can be used for bucket or object operations.
...
| Compares to the delimiter parameter specified in a ListObjects or ListObjectVersions request.
...
s3:PutObjectVersionTagging
| Requires that the existing object has the specific tag key and value.
...
s3:ListBucketVersions permissions
| Compares to the max-keys parameter specified in a ListObjects or ListObjectVersions request.
...
s3:ListBucketVersions permissions
| Compares to the prefix parameter specified in a ListObjects or ListObjectVersions request, or the key specified for GetObject or HeadObject. 
...
s3:PutObjectVersionTagging
| Requires a specific tag key and value when the object request includes tagging.

| s3:x-amz-copy-source
| s3:PutObject
| Compares to the x-amz-copy-source expanded from the request header in the PutObject and UploadPartCopy request.
...
| Bucket
| Valid. Objects are owned by the creator account, and the bucket policy doesn't apply. The creator account must grant access permissions for the object using object ACLs.
...
NOTE: Setting DeleteObject to DENY in an S3 policy doesn't prevent ILM from deleting objects when a rule such as "zero copies after 30 days" exists.
...
// 2026 APR 30, SGRIDDOC-246
// 2026 MAR 23, SGRIDDOC-231
// 2023 OCT 5, SGWS-27457
// 2025 May 14, SGWS-34003
// 2025 July 23, SGRIDDOC-180