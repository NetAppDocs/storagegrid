...
A session policy is an access policy that temporarily restricts the permissions available during a specific session, such as when a user assumes a group. The group itself might have broader permissions. 
