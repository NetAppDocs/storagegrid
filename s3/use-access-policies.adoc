...
A session policy is an access policy that restricts the permissions available during a specific session, such as when a user assumes a group. The group itself might have broader permissions. A session policy's temporary, more granular permission settings further restrict the role's standard permissions.
