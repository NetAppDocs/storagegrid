...
There are three kinds of access policies supported by StorageGRID.
...
A session policy is an access policy that restricts the permissions available during a specific session, such as when a user assumes a group. It is an inline policy passed during the session, setting a limit on the overall permissions available, even if the group itself has broader permissions. Think of it as a temporary, more granular permission set that overrides the role's standard permissions for that specific session.
