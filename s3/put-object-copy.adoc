...
+
When you copy an object, if the source object has a `checksum` or includes `x-amz-checksum-algorithm`, StorageGRID stores the copied object with a FULL_OBJECT checksum. 
+
If the source object has a checksum but doesn't include `x-amz-checksum-algorithm` or it has an unsupported value, the copied object has a checksum of the same type as the source object. 
+
If the source object doesn't have a checksum and doesn't include `x-amz-checksum-algorithm` or it has an unsupported value, StorageGRID doesn't store the copied object with a checksum.
