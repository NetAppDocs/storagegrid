...
+
When you copy an object, if the source object has a `checksum` or `x-amz-checksum-algorithm` is included, StorageGRID stores the copied object with a FULL_OBJECT checksum. If you omit `x-amz-checksum-algorithm` or use it with an unsupported value, the copied object has a checksum of the same type as the source object. 
+
If the source object does not have a checksum and you omit `x-amz-checksum-algorithm` or use it with an unsupported value, StorageGRID does not store the copied object with a checksum.
