...
+
If the source object has a checksum but the request doesn't include `x-amz-checksum-algorithm` or it uses an unsupported checksum value, the copied object has the same type of checksum as the source object. 
+
