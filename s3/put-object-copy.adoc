...
+
If the source object has a `checksum` or `x-amz-checksum-algorithm` is included when you copy an object, StorageGRID stores the copied object with a FULL_OBJECT checksum.
* `x-amz-copy-source`
