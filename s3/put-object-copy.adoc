...
+
The `x-amz-checksum-sha256` header is not supported for StorageGRID. In addition, when you copy an object, if the source object has a checksum, that checksum value is not copied to the new object.
