...
* `Content-Type`
* `x-amz-checksum-algorithm`: Specify `CRC-32`, `CRC-32C`, `CRC-64NVME`, or `SHA256`
+
When you copy an object, if the source object has a `checksum` or `x-amz-checksum-algorithm` is included, StorageGRID stores the copied object with a FULL_OBJECT checksum.
* `x-amz-copy-source`
