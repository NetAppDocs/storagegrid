...
---
= Create a copy of an object in StorageGRID with the S3 CopyObject request
:icons: font
:imagesdir: ../media/
...
StorageGRID doesn't parse or interpret escaped UTF-8 characters included in the key name or value of user-defined metadata. Escaped UTF-8 characters are treated as ASCII characters:

* Requests succeed if user-defined metadata includes escaped UTF-8 characters.
* StorageGRID doesn't return the `x-amz-missing-meta` header if the interpreted value of the key name or value includes unprintable characters.
...
* `Content-Type`
* `x-amz-checksum-algorithm`: Specify `CRC32`, `CRC32C`, `CRC64NVME`, or `SHA256`
+
When you copy an object, if the source object has a `checksum` or includes `x-amz-checksum-algorithm`, the copied object has a FULL_OBJECT checksum. 
+
If the source object has a checksum but the request doesn't include `x-amz-checksum-algorithm` or it uses an unsupported checksum value, the copied object has the same type of checksum as the source object. StorageGRID ignores invalid or unsupported `x-amz-checksum-algorithm` values.
+
If the source object doesn't have a checksum and doesn't include `x-amz-checksum-algorithm` or it uses an unsupported checksum value, the copied object doesn't have a checksum.
+
NOTE: If a CopyObject request doesn't specify a checksum and the source object doesn't have a checksum, StorageGRID doesn't calculate a default CRC-64/NVME checksum for an object.
...
* `Accept-Encoding`
+
When an object doesn't store a `Content-Encoding` value, a GET request with `Accept-Encoding: gzip` automatically performs compression. This only affects `gzip` values, other values are not affected. StorageGRID doesn't support Range read operations for GET requests with `Accept-Encoding: gzip`.
* `Cache-Control`
* `Content-Disposition`
* `Content-Encoding`
* `Content-Language`
...
* `If-Match`
+
The `If-Match` header behaves differently depending on your StorageGRID configuration. Providing this header might return an error message (501 Not Implemented). If the `If-Match` header is accepted but non-functional, an alert might trigger with instructions about enabling the stricter 501 Not Implemented error behavior.

* `If-None-Match`
+
The `If-None-Match` header behaves differently depending on your StorageGRID configuration. Providing this header might return an error message (501 Not Implemented). If the `If-None-Match` header is accepted but non-functional, an alert might trigger with instructions about enabling the stricter 501 Not Implemented error behavior.
...
If the source and destination match, and the `x-amz-metadata-directive` header is specified as `REPLACE`, the object's metadata is updated with the metadata values supplied in the request. In this case, StorageGRID doesn't re-ingest the object. This has two important consequences:
...
* The option for Ingest Behavior specified in the matching ILM rule isn't used. Any changes to object placement that are triggered by the update are made when ILM is re-evaluated by normal background ILM processes.
+
