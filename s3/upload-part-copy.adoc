...
---
= Upload a part of an object in StorageGRID with the S3 UploadPartCopy operation
:icons: font
:imagesdir: ../media/
...
StorageGRID automatically detects if a checksum was specified during a CreateMultipartUpload request and uses this checksum in the UploadPartCopy operation. You don't need to specify a checksum in an UploadPartCopy request. If a CreateMultipartUpload request doesn't specify a checksum, StorageGRID doesn't calculate a default CRC-64/NVME checksum for an object. StorageGRID doesn't include a checksum for the copied part when CreateMultipartUpload hasn't specified a checksum.
