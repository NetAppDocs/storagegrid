...
+
The `x-amz-checksum-sha256` is not supported for StorageGRID. However, when you copy an object using AWS, if the source object has a checksum, that checksum value is copied to the new object by default.
