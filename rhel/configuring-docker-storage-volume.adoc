...
NOTE: Podman is replacing Docker as the supported container run-time tool for software-only deployments. Prepare for this change by using Podman for new StorageGRID 11.9 installations. If you're using Docker with an earlier version of StorageGRID, you won't be able to upgrade to future releases until you convert to Podman only.
